...
static auto areconflictingkeyspressed(tsprite& spritec) -> bool
{
	...
	bool result = false;
	result =
		static_cast<std::int32_t>(spritec.control.thrownade) + static_cast<std::int32_t>(spritec.control.changeweapon)
			+ static_cast<std::int32_t>(spritec.control.throwweapon) + static_cast<std::int32_t>(spritec.control.reload)
		> 1;
	return result;
}
	...
	switch (spritec.style)
	{
		case tsprite::Style::Default: // Gostek
		{
			...
			if ((sprite_system.IsPlayerSprite(spritec.num)) && !gGlobalStateGameMenus.escmenu->active)
			{
				...
				sprite_system.GetPlayerSprite().control.mouseaimx =
					std::round(gGlobalStateClientGame.mx - gGlobalStateGame.gamewidthhalf + gGlobalStateClient.camerax);
				...
				if (!gGlobalStateGameMenus.teammenu->active && !gGlobalStateGameMenus.limbomenu->active)
				{
					if (gGlobalStateClientGame.chattext.empty())
					{
						...
						{
							if (gGlobalStateInput.keystatus[gGlobalStateInput.binds[i].keyid]
								and ((gGlobalStateInput.binds[i].keymod == 0)
									 || (static_cast<int>(
											 (gGlobalStateInput.binds[i].keymod != 0U) && (SDL_GetModState() != 0U))
										 != 0)))
							{
								...
								{
									spritec.control.flagthrow = spritec.control.up && spritec.control.down;
								}
							}
						}
				...
				if (freecampressed and !(spritec.control.fire || spritec.control.jetpack || spritec.control.up))
				{
								...
								if (!gGlobalStateDemo.demoplayer.active() or (gGlobalStateClient.freecam == 1))
								{
									...
									for (auto& sprite : sprite_system.GetActiveSprites())
									{
										if (sprite.isnotspectator()
											and (sprite.isinsameteam(sprite_system.GetPlayerSprite())
												 or sprite_system.GetPlayerSprite().isspectator()))
										{
											i = 1;
											break;
										}
									...
									cameratarget = iif(i == 1,
										gGlobalStateClientGame.getcameratarget(spritec.control.jetpack),
				...
				if (CVar::sv_realisticmode)
				{
					...
					{
						// Following sprites
						if ((spritec.num != gGlobalStateClient.camerafollowsprite) and spritec.isnotinsameteam(sprite)
						...
						{
							if (spritec.deadmeat
								or ((GS::GetGame().isteamgame() && sprite.isinsameteam(spritec))
									or ((!GS::GetGame().isteamgame()) && sprite.isnotinsameteam(spritec))))
							{
								sprite.visible = 45;
							}
							else
			...
			spritec.control.mouseaimx = round(spritec.control.mouseaimx + spriteVelocity.x);
			spritec.control.mouseaimy = round(spritec.control.mouseaimy + spriteVelocity.y);

			// use weapons
			b.x = 0;
			b.y = 0;

			if (spritec.control.jetpack
				and (((spritec.legsanimation.id == AnimationType::JumpSide)
						 && (((spritec.direction == -1) && spritec.control.right)
							 or ((spritec.direction == 1) && spritec.control.left) or playerpressedleftright))
					 or ((spritec.legsanimation.id == AnimationType::RollBack) && spritec.control.up)))
			{
			...
			else if (spritec.control.jetpack && (spritec.jetscount > 0))
			{
				...
				else
				{
					...
					{
						spriteForces.y = spriteForces.y - iif(grav > 0.05, jetspeed, grav * 2);
					}
					else
					{
						spriteForces.x =
				...
				if (spritec.position == pos_stand)
				{
					if ((spritec.control.fire) && (spritec.ceasefirecounter < 0))
					{
							...
							&& (spritec.weapon.num != chainsaw_num))
						{
							for (auto& sprite : sprite_system.GetActiveSprites())
							{
								if (!sprite.deadmeat and (sprite.position == pos_stand) && (sprite.num != spritec.num)
									...
									if (spritec.weapon.startuptime > 0)
									{
										...
										if (spritec.weapon.startuptimecount > 0)
										{
											...
											if (spritec.weapon.startuptimecount == spritec.weapon.startuptime)
											{
												...
												{
													gGlobalStateSound.playsound(SfxEffect::law_start,
														spritePartsPos,
												...
												{
													gGlobalStateSound.playsound(SfxEffect::minigun_start,
														spritePartsPos,
														spritec.gattlingsoundchannel);
												...
												{
													if (spritec.onground
														and (((spritec.legsanimation.id == AnimationType::Crouch)
											...
											if ((spritec.weapon.num != law_num)
												|| ((spritec.onground or spritec.ongroundpermanent)
													and (((spritec.legsanimation.id == AnimationType::Crouch)
															 && (spritec.legsanimation.currframe > 13))
														 || (spritec.legsanimation.id == AnimationType::CrouchRun)
							...
							if (spritec.weapon.startuptimecount < spritec.weapon.startuptime)
							{
								if (spritec.weapon.num == minigun_num)
								{
									...
									gGlobalStateSound.playsound(
										SfxEffect::minigun_end, spritePartsPos, spritec.gattlingsoundchannel2);
								}
								if (spritec.weapon.num == law_num)
								{
									if ((spritec.onground)
												 ...
												 && (spritec.legsanimation.currframe > 23))))
									{
										// LAW wind down sound
										gGlobalStateSound.playsound(
											SfxEffect::law_end, spritePartsPos, spritec.gattlingsoundchannel2);
									}
								}
							}
			...
			switch (spritec.weapon.firemode)
			{
				case 2: // Single shot
				{
					...
					if (spritec.player->controlmethod == human)
					{
						if (spritec.control.fire)
						{
							if (((spritec.burstcount > 0) or spritec.control.reload)
								and (spritec.weapon.fireintervalcount < 2))
							{
				...
				// TARGET MODE
#ifndef SERVER
			if ((sprite_system.IsPlayerSprite(spritec.num)) && gGlobalStateClient.targetmode && spritec.control.fire)
			{
				clientfreecamtarget();
			...
			if ((spritec.bodyanimation.id != AnimationType::Roll)
				&& (spritec.bodyanimation.id != AnimationType::RollBack) && (spritec.bonusstyle != bonus_flamegod))
			{
			...
			if (spritec.control.throwweapon && !spritec.control.thrownade && !spritec.dontdrop
				and (spritec.bodyanimation.id != AnimationType::Roll)
				&& (spritec.bodyanimation.id != AnimationType::RollBack)
			...
			{
#ifndef SERVER
				gGlobalStateSound.playsound(SfxEffect::spas12_reload, spritePartsPos, spritec.reloadsoundchannel);
#endif
			...
			if (spritec.weapon.num != knife_num)
			{
				if ((spritec.bodyanimation.id == AnimationType::ThrowWeapon) && (spritec.bodyanimation.currframe == 19)
			...
			// Throw knife
			if ((spritec.weapon.num == knife_num) && (spritec.bodyanimation.id == AnimationType::ThrowWeapon)
				&& (!spritec.control.throwweapon or (spritec.bodyanimation.currframe == 16)))
			{
...
#endif
				{
					...
					gGlobalStateClientGame.lastforceclientspritesnapshotmovtick = GS::GetGame().GetMainTickCounter();
#endif
					...
					createbullet(
						a, b, guns[thrownknife].num, spritec.num, 255, guns[thrownknife].hitmultiply, true, false);
			...
			if (!spritec.deadmeat)
			{
					...
					&& (spritec.weapon.num != law_num) && (spritec.weapon.num != m79_num))
				{
					...
					b.y = 0;
					createbullet(a, b, spritec.weapon.num, spritec.num, 255, spritec.weapon.hitmultiply, true, false);
			...
			// M79 luska
			if ((spritec.weapon.num == m79_num) && (spritec.weapon.reloadtimecount == spritec.weapon.clipouttime))
			{
#ifndef SERVER
				b = spritec.gethandsaimdirection();
				vec2scale(b, b, spritec.weapon.speed);
			...
			if (spritec.position == pos_prone)
			{
				...
				{
					if (((spritec.legsanimation.id == AnimationType::Prone) && (spritec.legsanimation.currframe > 23))
						|| (spritec.legsanimation.id == AnimationType::ProneMove))
					{
						if (spritec.legsanimation.id != AnimationType::GetUp)
						{
			...
			if ((spritec.legsanimation.id == AnimationType::GetUp) && (spritec.legsanimation.currframe > 23 - (4 - 1))
				&& // Possible during the last 4 frames
				spritec.onground && spritec.control.up and (spritec.control.right or spritec.control.left))
			{
				...
				unprone = true;
					 ...
					 spritec.onground and spritec.control.up && !(spritec.control.right or spritec.control.left))
			{
			...
			if ((spritec.weapon.num == barrett_num) && (spritec.weapon.fireintervalcount > 0))
			{
				if ((spritec.bodyanimation.id == AnimationType::Stand)
					|| (spritec.bodyanimation.id == AnimationType::Crouch)
			...
			{
				if (((spritec.bodyanimation.id == AnimationType::Stand)
						&& (spritec.legsanimation.id == AnimationType::Stand) and !spritec.deadmeat
						and (spritec.idletime > 0))
			...
			else if (spritec.idlerandom == 1) // CIGAR
			{
				if (!spritec.deadmeat)
				{
					...
					if ((spritec.bodyanimation.id == AnimationType::Cigar) && (spritec.bodyanimation.currframe == 37))
					{
					...
					if ((spritec.bodyanimation.id == AnimationType::Cigar) && (spritec.bodyanimation.currframe == 9))
					{
						if (spritec.hascigar == 5)
						{
							// Step 4/8
					...
					if ((spritec.bodyanimation.id == AnimationType::Cigar) && (spritec.bodyanimation.currframe == 26))
					{
						if (spritec.hascigar == 5)
						{
							// Step 5/8
					...
					if ((spritec.bodyanimation.id == AnimationType::Smoke) && (spritec.bodyanimation.currframe == 38))
					{
						// Step 8/8
						spritec.hascigar = 0;
#ifndef SERVER
			...
			else if (spritec.idlerandom == 7) // SELFKILL
			{
				if (spritec.idletime == 0)
				{
					if (spritec.canmercy)
					{
						if ((spritec.weapon.num == m79_num) || (spritec.weapon.num == m249_num)
							|| (spritec.weapon.num == spas12_num) || (spritec.weapon.num == law_num)
							|| (spritec.weapon.num == chainsaw_num) || (spritec.weapon.num == barrett_num)
				...
				if ((spritec.bodyanimation.id == AnimationType::Mercy)
					|| (spritec.bodyanimation.id == AnimationType::Mercy2))
				{
					if (spritec.bodyanimation.currframe == 20)
					{
						...
						if (spritec.weapon.num == knife_num)
						{
							gGlobalStateSound.playsound(SfxEffect::slash,
								spritePartsPos,
								sprite_system.GetSprite(spritec.num).gattlingsoundchannel);
						}
						if (spritec.weapon.num == chainsaw_num)
						{
							gGlobalStateSound.playsound(SfxEffect::chainsaw_r,
								spritePartsPos,
								sprite_system.GetSprite(spritec.num).gattlingsoundchannel);
						}
						if (spritec.weapon.num == noweapon_num)
						{
							gGlobalStateSound.playsound(SfxEffect::dead_hit,
								spritePartsPos,
								sprite_system.GetSprite(spritec.num).gattlingsoundchannel);
						}
			...
			if (spritec.legsanimation.speed > 1)
			{
				if ((spritec.legsanimation.id == AnimationType::Jump)
					...
					|| (spritec.legsanimation.id == AnimationType::Prone)
					|| (spritec.legsanimation.id == AnimationType::Run)
					|| (spritec.legsanimation.id == AnimationType::RunBack))
				{

					spriteVelocity.x = ((float)(spriteVelocity.x) / spritec.legsanimation.speed);
					spriteVelocity.y = ((float)(spriteVelocity.y) / spritec.legsanimation.speed);
				}
			...
			if (spritec.weapon.num == barrett_num)
			{
				if ((spritec.weapon.fireintervalcount == 0)
					&& ((spritec.bodyanimation.id == AnimationType::Prone)
						|| (spritec.bodyanimation.id == AnimationType::Aim)))
				{
					if ((fabs(spritec.control.mouseaimx - spritePartsPos.x) >= 640 / 1.035)
						|| (fabs(spritec.control.mouseaimy - spritePartsPos.y) >= 480 / 1.035))
					{
					...
					if ((fabs(spritec.control.mouseaimx - spritePartsPos.x) < 640 / 1.5)
			...
			if ((spritec.bodyanimation.id == AnimationType::Cigar) || (spritec.bodyanimation.id == AnimationType::Match)
				|| (spritec.bodyanimation.id == AnimationType::Smoke)
				|| (spritec.bodyanimation.id == AnimationType::Wipe)
				|| (spritec.bodyanimation.id == AnimationType::Groin))
			{
				if (spritec.control.left or spritec.control.right or spritec.control.up or spritec.control.down
					or spritec.control.fire or spritec.control.jetpack or spritec.control.thrownade
					or spritec.control.changeweapon or spritec.control.throwweapon or spritec.control.reload
					or spritec.control.prone)
				{
			...
			// rolling
			if ((spritec.bodyanimation.id != AnimationType::TakeOff)
				&& (spritec.bodyanimation.id != AnimationType::Piss)
				&& (spritec.bodyanimation.id != AnimationType::Mercy)
				&& (spritec.bodyanimation.id != AnimationType::Mercy2)
				&& (spritec.bodyanimation.id != AnimationType::Victory)
				&& (spritec.bodyanimation.id != AnimationType::Own))
			{
				if ((spritec.bodyanimation.id == AnimationType::Roll)
					|| (spritec.bodyanimation.id == AnimationType::RollBack))
				{
					...
					if (spritec.onground)
					{
							...
							|| (spritec.legsanimation.id == AnimationType::ProneMove))
						{
							if (spritec.control.left or spritec.control.right)
							{
								...
								{
									spriteForces.x = iif(spritec.control.left, -pronespeed, pronespeed);
								}
			...
			if ((spritec.bodyanimation.id == AnimationType::RollBack)
				&& (spritec.legsanimation.id != AnimationType::RollBack))
			{
				spritec.legsapplyanimation(AnimationType::RollBack, 1);
			}
			...
			if (spritec.weapon.ammocount > 0)
			{
				if ((!spritec.control.thrownade && (spritec.bodyanimation.id != AnimationType::Recoil)
						...
						&& (spritec.bodyanimation.id != AnimationType::ReloadBow)
						&& (spritec.bodyanimation.id != AnimationType::Cigar)
						&& (spritec.bodyanimation.id != AnimationType::Match)
						&& (spritec.bodyanimation.id != AnimationType::Smoke)
						&& (spritec.bodyanimation.id != AnimationType::Wipe)
						...
						&& (spritec.bodyanimation.id != AnimationType::Reload)
						&& (spritec.bodyanimation.id != AnimationType::Prone)
						&& (spritec.bodyanimation.id != AnimationType::GetUp)
						&& (spritec.bodyanimation.id != AnimationType::ProneMove)
						&& (spritec.bodyanimation.id != AnimationType::Melee))
					|| ((spritec.bodyanimation.currframe == spritec.bodyanimation.numframes)
						&& (spritec.bodyanimation.id != AnimationType::Prone))
					|| ((spritec.weapon.fireintervalcount == 0) && (spritec.bodyanimation.id == AnimationType::Barret)))
				{
					if (spritec.position != pos_prone)
					{
						if (spritec.position == pos_stand)
			...
			if ((spritec.legsanimation.id == AnimationType::Crouch)
				|| (spritec.legsanimation.id == AnimationType::CrouchRun)
				|| (spritec.legsanimation.id == AnimationType::CrouchRunBack))
			{
