...
auto creatething(tvector2 spos, std::uint8_t owner, std::uint8_t sstyle,
          ...
          if (sprite_system.GetSprite(j).bodyanimation.id != AnimationType::Change)
          {
            if (timeout < gunresisttime - 30)
            {
              ...
              scrptdispatcher.onweaponchange(
                j, guns[weaponindex].num, SpriteSystem::Get().GetSprite(j).secondaryweapon.num,
    ...
    case object_flamer_kit:
          ...
          (sprite_system.GetSprite(j).ceasefirecounter < 1))
      {
            ...
            (sprite_system.GetSprite(j).weapon.num != bow2_num))
        {
          ...
          forceweaponcalled = false;
          scrptdispatcher.onweaponchange(
            j, flamer_num, SpriteSystem::Get().GetSprite(j).secondaryweapon.num, ammocount,
          ...
          if (sprite.control.fire)
          {
            if (sprite.legsanimation.id == AnimationType::Stand)
            {
              if (GS::GetGame().GetMainTickCounter() % guns[m2].fireinterval == 0)
              {
                ...
                createbullet(pos, norm, guns[m2].num, sprite.num, 255, guns[m2].hitmultiply, true,
                             false);
