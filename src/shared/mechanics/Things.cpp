...
auto creatething(tvector2 spos, std::uint8_t owner, std::uint8_t sstyle, std::uint8_t n) -> std::int32_t

{
	...
	// Throw weapon
	if (((sstyle > object_pointmatch_flag) && (sstyle < object_medical_kit)) || (sstyle == object_law)
	...
	if (!statictype)
	{
		...
		if (style < object_ussocom)
		{
			if ((holdingsprite > 0) && (holdingsprite < max_sprites + 1))
			{
				...
				{
					bgstate.backgroundstatus = sprite_system.GetSprite(holdingsprite).bgstate.backgroundstatus;
						...
						if (distance(skeleton.pos[1], things[i].skeleton.pos[1]) < touchdown_radius)
						{
							if (sprite_system.GetSprite(holdingsprite).player->team == team_alpha)
							{
								...
								if (CVar::sv_gamemode == gamestyle_inf)
								{
									GS::GetGame().SetTeamScore(
										1, GS::GetGame().GetTeamScore(1) + CVar::sv_inf_redaward - 1);
									// penalty
									if (GS::GetGame().GetPlayersTeamNum(1) > GS::GetGame().GetPlayersTeamNum(2))
									{
										GS::GetGame().SetTeamScore(1,
											GS::GetGame().GetTeamScore(1)
												- (5
								...
								if (sprite_system.GetSprite(holdingsprite).player->controlmethod == bot)
								{
									if (Random(sprite_system.GetSprite(holdingsprite).brain.chatfreq / 3) == 0)
									{
										serversendstringmessage(
											(sprite_system.GetSprite(holdingsprite).brain.chatwinning),
											all_players,
											holdingsprite,
											msgtype_pub);
									}
							...
							if (CVar::sv_survivalmode)
							{
								...
								for (auto& sprite : SpriteSystem::Get().GetActiveSprites())
								{
									...
									if (!sprite.deadmeat)
									{
										sprite.healthhit(hit, sprite.num, 1, -1, sprite.skeleton.pos[12]);
...
void Thing<M>::render(double timeelapsed)
{
	...
	if (gGlobalStateClient.gamethingtarget == num)
	{
		...
		gfxdrawsprite(t[GFX::OBJECTS_ILUM], _p.x, _p.y, rgba(0xffffff, round(fabs(5 + (20 * sin(5.1 * timeelapsed))))));
	}
	...
	switch (style)
	{
		...
		case object_pointmatch_flag: // Flag
		{
			...
			if (inbase)
			{
				...
				_p.y = pos1->y + (static_cast<float>((pos2->y - pos1->y)) / 2) - 12.5;
				gfxdrawsprite(
					t[GFX::OBJECTS_ILUM], _p.x, _p.y, rgba(0xffffff, round(fabs(5 + (20 * sin(5.1 * timeelapsed))))));
			}
		}
		break; // 1,2,3
		...
		case object_parachute: // para
		{
			...
			gfxdrawsprite(t[tex2 + 1], _p.x, _p.y, _scala.x, _scala.y, _ra.x, _ra.y, -roto, rgba(color));
		...
		case object_stationary_gun: // stat gun
		{
			...
			if (pos4->x >= pos1->x)
			{
				gfxdrawsprite(t[GFX::WEAPONS_M2_2],
					_p.x,
					_p.y,
					_ra.x,
					_ra.y,
			...
			{
				gfxdrawsprite(t[GFX::WEAPONS_M2],
					_p.x,
					_p.y,
					_ra.x,
					_ra.y,
...
auto Thing<M>::checkmapcollision(std::int32_t i, float x, float y) -> bool
{
	...
	if (sector.IsValid())
	{
		...
		for (const auto& w : sector.GetPolys())
		{
			...
			if (teamcol && (polytype != poly_type_only_bullets) && (polytype != poly_type_only_player)
				&& (polytype != poly_type_doesnt) && (polytype != poly_type_only_flaggers)
...
auto Thing<M>::checkspritecollision() -> std::int32_t
{
	...
	for (auto& sprite : sprite_system.GetActiveSprites())
	{
		if (!sprite.deadmeat && sprite.isnotspectator())
		{
			...
			if (dist < radius)
			{
				if (dist < closestdist)
				{
					if ((style != object_medical_kit) || (sprite.GetHealth() != GS::GetGame().GetStarthealth()))
					{
						if ((style != object_grenade_kit) || !(sprite.tertiaryweapon.ammocount == CVar::sv_maxgrenades)
	...
	if (j > 0) // collision
	{
		if (((((style > object_pointmatch_flag) && (style < object_rambo_bow)
				...
				&& (sprite_system.GetSprite(j).brain.favweapon != noweapon_num) && (timeout < gunresisttime - 30))
		...
		switch (style)
		{
					...
					if (sprite_system.GetSprite(j).brain.favweapon != noweapon_num)
					{
						if (sprite_system.GetSprite(j).bodyanimation.id != AnimationType::Change)
						{
							if (timeout < gunresisttime - 30)
							{
								...
								scrptdispatcher.onweaponchange(j,
									guns[weaponindex].num,
									SpriteSystem::Get().GetSprite(j).secondaryweapon.num,
									ammocount,
			...
			break;
			case object_berserk_kit:
				if ((sprite_system.GetSprite(j).bonusstyle == 0) && (sprite_system.GetSprite(j).ceasefirecounter < 1))
				{
					...
					if (sprite_system.GetSprite(j).brain.favweapon != noweapon_num)
					{
						if (sprite_system.GetSprite(j).bodyanimation.id != AnimationType::Change)
						{
							if (timeout < gunresisttime - 30)
							{
								...
								weaponindex = weaponnumtoindex(style - (object_num_nonweapon + 1), guns);
	#ifdef SCRIPT
								// event must be before actual weapon apply.
								// script might've called ForceWeapon, which we should check.
								// if it did, we don't apply snapshot weapon's as they were already
...
#ifdef SERVER
template <Config::Module M>
auto Thing<M>::checkstationaryguncollision() -> std::int32_t
#else
template <Config::Module M>
					...
					if (sprite.control.fire)
					{
						if (sprite.legsanimation.id == AnimationType::Stand)
						{
							if (GS::GetGame().GetMainTickCounter() % guns[m2].fireinterval == 0)
							{
								...
								createbullet(
									pos, norm, guns[m2].num, sprite.num, 255, guns[m2].hitmultiply, true, false);
...
template std::int32_t creatething(tvector2 spos, std::uint8_t owner, std::uint8_t sstyle, std::uint8_t n);
