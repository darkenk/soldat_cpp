...
template <Config::Module M>
auto createbullet(tvector2 spos,
	tvector2 svelocity,
	std::uint8_t snum,
	std::int32_t sowner,
	std::uint8_t n,
	...
	std::uint16_t seed) -> std::int32_t // Seed = -1
{
	...
	if (net)
	{
		if ((sprite_system.IsPlayerSprite(sowner)) && (gGlobalStateClientGame.clientstopmovingcounter > 0))
		{
	...
	if (net)
	{
		...
		else
		{
			...
			{
				if ((sprite.player->controlmethod == human) && (sprite.num != b.owner) && (sprite.player->port == 0)
					&& sprite.isspectator())
				{
...
#ifdef SERVER
auto servercreatebullet(
	tvector2 spos, tvector2 svelocity, std::uint8_t snum, std::int32_t sowner, std::uint8_t n, float hitm, bool net)
	-> std::int32_t
{
	std::int32_t i = 0;
...
auto bulletcansend(float x, float y, const std::int32_t i, float vx) -> bool
{
	...
	sx = spritePartsPos.x - (static_cast<float>((spritePartsPos.x - sprite_system.GetSprite(i).control.mouseaimx)) / 2);
	sy = spritePartsPos.y - (static_cast<float>((spritePartsPos.y - sprite_system.GetSprite(i).control.mouseaimy)) / 2);
#ifdef SERVER
	if ((x > (sx - max_game_width)) && (x < (sx + max_game_width)) && (y > (sy - 480)) && (y < (sy + 480)))
	{
		result = true;
	}
...
auto canhitspray(std::int32_t victim, std::int32_t attacker) -> bool
{
	...
	{
		if ((victim == attacker) or (CVar::sv_friendlyfire) or sprite_system.GetSprite(victim).issolo()
			or sprite_system.GetSprite(victim).isnotinsameteam(sprite_system.GetSprite(attacker)))
		{
			result = true;
		}
	}
...
void calculaterecoil(float px, float py, float& cx, float& cy, float da)
{
	...
	cx = px
	   + ((dx + std::cos(alpha) * radius) / sprite_system.GetSprite(gGlobalStateClient.camerafollowsprite).aimdistcoef)
			 * (sprite_system.GetSprite(gGlobalStateClient.camerafollowsprite).aimdistcoef / 2);
	cy = py
	   + ((dy + std::sin(alpha) * radius) / sprite_system.GetSprite(gGlobalStateClient.camerafollowsprite).aimdistcoef)
			 * (sprite_system.GetSprite(gGlobalStateClient.camerafollowsprite).aimdistcoef / 2);
...
void Bullet<M>::update()
{
	...
	if (sprite_system.IsPlayerSprite(tracking))
	{
		...
		{
			gGlobalStateClient.camerax = GetBulletParts().pos[num].x + 5 * GetBulletParts().velocity[num].x;
			gGlobalStateClient.cameray = GetBulletParts().pos[num].y + 5 * GetBulletParts().velocity[num].y;
		}
		else
		{
			tracking = 0;
	...
	if (!whizzed)
	{
		if (style != bullet_style_punch)
		{
			...
			{
				const auto& spritePartsPos = sprite_system.GetSpritePartsPos(gGlobalStateClient.camerafollowsprite);
	...
	if (style == bullet_style_law)
	{
		...
		if (Random(2) == 0)
		{
			gGlobalStateSparks.createspark(GetBulletParts().pos[num], GetBulletParts().velocity[num], 2, num, 5);
		}
	}
#endif
	...
	if (hitbody > 0)
	{
		if (Random(5) == 0)
		{
			gGlobalStateSparks.createspark(GetBulletParts().pos[num],
				vector2(GetBulletParts().velocity[num].x * 0.5, GetBulletParts().velocity[num].y * 0.5),
				4,
				owner,
				90);
		}
...
void Bullet<M>::render(double timeelapsed)
{
	...
	if (CVar::sv_realisticmode)
	{
		if ((owner > 0) && (owner < max_sprites + 1))
		{
			if (sprite_system.GetSprite(owner).IsActive())
			{
				...
				{
					if (map.raycast(bulletpos,
							sprite_system.GetPlayerSprite().skeleton.pos[9],
							grenvel,
							gGlobalStateGame.gamewidth,
							true))
					{
						return;
					}
	...
	switch (style)
	{
		case bullet_style_plain: // bullet
			...
			{
				if ((imagestyle != guns[eagle].bulletimagestyle) && (imagestyle != guns[mp5].bulletimagestyle)
					&& (imagestyle != guns[ak74].bulletimagestyle) && (imagestyle != guns[steyraug].bulletimagestyle)
					&& (imagestyle != guns[ruger77].bulletimagestyle) && (imagestyle != guns[barrett].bulletimagestyle)
					&& (imagestyle != guns[m249].bulletimagestyle) && (imagestyle != guns[minigun].bulletimagestyle)
				...
				{
					gfxdrawsprite(
						t[imagestyle], _p.x, _p.y, _scala.x, _scala.y, 0, 0, roto, rgba(0xffffff, std::round(alfa)));
				}
				...
				if (pingadd > 0)
				{
					...
					b.x = vec2length(a)
						* min(static_cast<float>(1) / bulletlength,
		...
		case bullet_style_fragnade:
		{
			...
			if (gGlobalStateClient.trails == 1)
			{
				if (timeoutfloat < grenade_timeout - 3)
				{
					...
					_scala.x = grenvel / 3;
					_scala.y = 1;
					gfxdrawsprite(t[GFX::WEAPONS_BULLET],
						_p.x,
						_p.y,
						_scala.x,
						_scala.y,
		...
		case bullet_style_m79:
			if (timeoutfloat < bullet_timeout - 2)
			{
				...
				_scala.y = 1;
				roto = -angle2points(_p, _p2);
				gfxdrawsprite(t[GFX::WEAPONS_M79_BULLET],
					_p.x,
					_p.y,
					_scala.x,
					_scala.y,
		...
		case bullet_style_clusternade:
		{
			_p.x = bulletpos.x;
			_p.y = bulletpos.y - 3;
			gfxdrawsprite(t[GFX::WEAPONS_CLUSTER_GRENADE],
				_p.x,
				_p.y,
				0,
				0,
			...
			if (timeoutfloat < m2bullet_timeout - 2)
			{
				...
				if (gGlobalStateClient.trails == 1)
				{
					if (timeoutfloat < m2bullet_timeout - 13)
					{
						...
						_scala.y = sinusvar;
						gfxdrawsprite(t[GFX::WEAPONS_SMUDGE],
							_p.x,
							_p.y,
							_scala.x,
							_scala.y,
...
auto Bullet<M>::checkmapcollision(float x, float y) -> tvector2
{
	...
	// make step
	largestvelocitycomponent = max(fabs(GetBulletParts().velocity[num].x), fabs(GetBulletParts().velocity[num].y));
									...
									if (sector.IsValid())
									{
										for (const auto& w2 : sector.GetPolys())
										{
											...
											if ((polytype != poly_type_only_player) && (polytype != poly_type_doesnt)
												...
												and teamcollides(
													polytype, sprite_system.GetSprite(owner).player->team, true))
											{
...
auto Bullet<M>::checkspritecollision(float lasthitdist) -> tvector2
{
	...
	auto& sprite_system = SpriteSystem::Get();
	const std::array<std::int32_t, 7> bodypartspriority = {
		{ 12, 11, 10, 6, 5, 4, 3 }
	};
	...
	tvector2 result;
	LogTraceG("TBullet.CheckSpriteCollision {} {} {} {} {}", style, owner, ownerweapon, timeout, tracking);
	...
	if (style != bullet_style_clusternade)
	{
		...
		while (spritecounter < spritecount)
		{
			...
			if ((style == bullet_style_punch) || (style == bullet_style_knife))
			{
				...
				startpoint = vec2add(sprite_system.GetSprite(owner).skeleton.pos[15], buttstockpositionoffset);
			...
			for (bodypartpriorityindex = 0; bodypartpriorityindex <= high(bodypartspriority); bodypartpriorityindex++)
			{
			...
			if (((style != bullet_style_punch) && (style != bullet_style_knife)) || (std::cmp_not_equal(j, owner)))
			{
				if (where > 0)
				{
					// order collision
					...
					if (((nocollision & weapon_nocollision_enemy) != 0)
						and (sprite_system.GetSprite(j).isnotinsameteam(sprite_system.GetSprite(owner))))
					{
						continue;
					}

					if (((nocollision & weapon_nocollision_team) != 0)
						and (sprite_system.GetSprite(j).isinsameteam(sprite_system.GetSprite(owner)))
					...
					if (sprite_system.GetSprite(j).ceasefirecounter < 0)
					{
						...
						switch (style)
						{
							...
							case bullet_style_m2:
							{
...
#ifndef SERVER
								// Blood spark
								if ((CVar::sv_friendlyfire) or sprite_system.GetSprite(owner).issolo()
									or sprite_system.GetSprite(owner).isnotinsameteam(sprite_system.GetSprite(j))
									or (std::cmp_equal(j, owner)))
								{
									hit(hit_type_blood);
								}
								...
								if (sprite_system.GetSprite(j).vest < 1)
								{
									if (!sprite_system.GetSprite(j).deadmeat)
									{
										gGlobalStateSound.playsound(
											SfxEffect::hit_arg + Random(3), GetBulletParts().pos[num]);
									}
									else
									{
								...
								if (style == bullet_style_punch)
								{
									if (sprite_system.GetSprite(j).issolo()
										or (sprite_system.GetSprite(j).isnotsolo()
											and sprite_system.GetSprite(j).isnotinsameteam(
												sprite_system.GetSprite(owner))))
									{
										if ((sprite_system.GetSprite(j).weapon.num != bow_num)
											&& (sprite_system.GetSprite(j).weapon.num != bow2_num))
										{
											sprite_system.GetSprite(j).bodyapplyanimation(
							...
							case bullet_style_arrow:
								if (timeout > arrow_resist)
								{
									...
									GetBulletParts().forces[num].y =
										GetBulletParts().forces[num].y - GetBulletParts().gravity;
									if (((CVar::sv_friendlyfire) && sprite_system.GetSprite(owner).isnotsolo()
											and sprite_system.GetSprite(owner).isinsameteam(sprite_system.GetSprite(j))
#ifndef SERVER
								...
								if (std::cmp_not_equal(owner, j))
								{
...
#endif
									{
										if (hitmultiply >= guns[flamer].hitmultiply / 3)
										{
											...
											createbullet(sprite_system.GetSprite(j).skeleton.pos[where],
												a,
												flamer_num,
												owner,
												255,
...
auto Bullet<M>::checkthingcollision(float lasthitdist) -> tvector2
{
	...
	if (style != bullet_style_fragnade)
	{
		...
		{
			if (things[j].active && (timeout < bullet_timeout - 1) && things[j].collidewithbullets
				and (owner != things[j].holdingsprite)
			...
			if (linecirclecollision(startpoint, endpoint, colpos, map.collider[j].radius / 1.7, pos))
			{
				...
				switch (style)
				{
					...
					case bullet_style_arrow:
						if (timeout > arrow_resist)
						{
							GetBulletParts().forces[num].y = GetBulletParts().forces[num].y - GetBulletParts().gravity;
							hit(hit_type_wall);
	...
	switch (t)
	{
		...
		case hit_type_clusternade:
		{
			...
			for (i = 1; i <= 5; i++)
			{
				...
				createbullet(a, b, cluster_num, owner, 255, guns[fraggrenade].hitmultiply / 2.0F, true, false);
			}
		}
		break;
...
void Bullet<M>::explosionhit(std::int32_t typ, std::int32_t spritehit, std::int32_t where)
{
	...
	const std::array<std::int32_t, 7> bodyparts = {
		{ 12, 11, 10, 6, 5, 4, 3 }
	};
#ifndef SERVER
	...
	for (auto& sprite : sprite_system.GetActiveSprites())
	{
		...
		if (((nocollision & weapon_nocollision_exp_enemy) != 0)
			and (sprite.isnotinsameteam(sprite_system.GetSprite(owner))))
		{
			continue;
		}

		if (((nocollision & weapon_nocollision_exp_team) != 0) and (sprite.isinsameteam(sprite_system.GetSprite(owner)))
			&& (sprite.num != owner))
		{
			continue;
		}
	...
	for (i = 1; i <= max_bullets; i++)
	{
		auto& b = bullet[i];
		if ((i != num) && b.active
			&& ((b.style == bullet_style_fragnade) || (b.style == bullet_style_m79) || (b.style == bullet_style_law)))
		{
...
auto Bullet<M>::filterspritesbydistance(tspriteindexes& spriteindexes) -> std::int32_t
{
	...
	for (i = 1; i <= max_sprites; i++)
	{
		if (targetablesprite(i))
		{
			...
			std::memmove(&spriteindexes[j + 1], &spriteindexes[j], (spritecount - j) * sizeof(std::int32_t));
			spriteindexes[j] = i;
		}
	}
...
auto Bullet<M>::targetablesprite(std::int32_t i) -> bool
{
	...
	result = sprite_system.GetSprite(i).active && ((std::cmp_not_equal(owner, i)) || (timeout < ownervulnerabletime))
