...
tvector2 Bullet<M>::checkmapcollision(float x, float y)
{
    ...
    for (b = 0; b <= detacc - 1; b++)
    {
        ...
        for (const auto &w : sector.GetPolys())
        {
            ...
            teamcol =
                teamcollides(polytype, SpriteSystem::Get().GetSprite(owner).player->team, true);
            if (teamcol)
                                ...
                                if (sector.IsValid())
                                {
                                    for (const auto &w2 : sector.GetPolys())
                                    {
                                        ...
                                        if ((polytype != poly_type_only_player) &&
                                            ...
                                            teamcollides(
                                                polytype,
                                                SpriteSystem::Get().GetSprite(owner).player->team,
                                                true))
    ...
    if (style != bullet_style_clusternade)
    {
        ...
        while (spritecounter < spritecount)
        {
            ...
            if ((style == bullet_style_punch) || (style == bullet_style_knife))
            {
                ...
                startpoint = vec2add(SpriteSystem::Get().GetSprite(owner).skeleton.pos[15],
                                     buttstockpositionoffset);
...
#ifndef SERVER
                            // Blood spark
                            if ((CVar::sv_friendlyfire) or
                                SpriteSystem::Get().GetSprite(owner).issolo() or
