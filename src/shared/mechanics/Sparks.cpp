...
void tspark::update()
{
    const std::set<std::int32_t> noneuler_style = {12, 13, 14, 15, 17, 24, 25, 28,
                                                   29, 31, 36, 37, 50, 54, 56, 60};

    const std::set<std::int32_t> collidable_style = {
...
void tspark::render()
{
        ...
        if ((owner > 0) && (owner < max_sprites + 1))
            if (sprite[owner].active)
                if (sprite[owner].visible == 0)
                    if (map.raycast(sparkparts.pos[num], sprite[mysprite].skeleton.pos[9], grenvel,
                                    gamewidth, true) or
                        (sprite[owner].visible == 0))
                        return;
    ...
    break;
    case 6:
        if (sprite[owner].player->headcap > 0)
            gfxdrawsprite(t[sprite[owner].player->headcap], _p.x, _p.y, 0, 0, degtorad(l * 2),
                          rgba(sprite[owner].player->shirtcolor));
        break;
    case 7:
    ...
    break;
    case 16:
        gfxdrawsprite(t[GFX::WEAPONS_SHELL], _p.x, _p.y, 1, 2, 0, 0, degtorad(l * 3.5),
                      rgba(0x8877ff));
        break;
    case 17: {
        _p.x = _p.x - 25;
    ...
    case 25:
        gfxdrawsprite(t[GFX::SPARKS_SPAWNSPARK], _p.x - 20, _p.y - 20, 0, 0, degtorad(l),
                      rgba(sprite[owner].player->shirtcolor, min(6.0 * l, 255.0)));
        break;
    case 26:
        gfxdrawsprite(t[GFX::SPARKS_ODPRYSK], _p.x, _p.y,
                      rgba(0xfffe35, min(l * 3 + 154.0, 255.0)));
        break;
    case 27:
        gfxdrawsprite(t[GFX::SPARKS_ODPRYSK], _p.x, _p.y,
                      rgba(0xaaaaaa, min(l * 3.0 + 154, 255.0)));
        break;
    case 28:
        gfxdrawsprite(t[GFX::SPARKS_EXPLOSION_EXPLODE16 - (std::int32_t)round((float)(l) / 3)],
    ...
    case 37: {
        ...
        _scala.y = (float)(l) / 75;
        _p.y = _p.y - ((float)(1) / _scala.y);
        gfxdrawsprite(t[GFX::SPARKS_BLACKSMOKE], _p.x, _p.y, _scala.x, _scala.y,
                      rgba(0xffffff, l * 3));
    }
    break;
    case 38:
    ...
    case 61:
        gfxdrawsprite(t[GFX::SPARKS_SPAWNSPARK], _p.x - 10, _p.y - 10, 0.5, 0.5, 0, 0, degtorad(l),
                      rgba(sprite[owner].player->shirtcolor, 14 * l));
        break;
    case 62:
