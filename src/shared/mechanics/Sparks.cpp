...
auto GlobalStateSparks::createspark(
	tvector2 spos, tvector2 svelocity, std::uint8_t sstyle, std::uint8_t sowner, std::int32_t life) -> std::int32_t
{
	...
	if (gGlobalStateClient.camerafollowsprite > 0)
	{
		...
		{
			if (!GS::GetGame().pointvisible(spos.x, spos.y, gGlobalStateClient.camerafollowsprite) && (sstyle != 38))
			{
				result = 0;
				return result;
			}
		...
		if (!sprite_system.IsPlayerSprite(gGlobalStateClient.camerafollowsprite))
		{
			if (!GS::GetGame().pointvisible2(spos.x, spos.y, gGlobalStateClient.camerafollowsprite) && (sstyle != 38))
			{
				result = 0;
				return result;
			}
	...
	for (i = 1; i <= CVar::r_maxsparks + 1; i++)
	{
		...
		if (!gGlobalStateGame.spark[i].active && (gGlobalStateGame.spark[i].style == 0) && !GetSparkParts().active[i])
		{
			result = i;
			break;
		}
...
{
	auto& sprite_system = SpriteSystem::Get();
	static const std::set<std::int32_t> collidable_style = { 2,
		4,
		5,
		6,
		7,
		...
		72,
		73 };
	static const std::set<std::int32_t> noneuler_style = {
	...
	{
		checkmapcollision(gGlobalStateSparks.GetSparkParts().pos[num].x, gGlobalStateSparks.GetSparkParts().pos[num].y);
	}
	...
	{
		gGlobalStateSparks.createspark(gGlobalStateSparks.GetSparkParts().pos[num], vector2(0, 0), 26, owner, 35);
	}
...
void tspark::render() const
{
			...
			if (sprite_system.GetSprite(owner).active)
			{
				if (sprite_system.GetSprite(owner).visible == 0)
				{
					if (map.raycast(gGlobalStateSparks.GetSparkParts().pos[num],
							...
							true)
						or (sprite_system.GetSprite(owner).visible == 0))
					{
						return;
					}
				}
	...
	switch (style)
	{
		...
		case 4:
			gfxdrawsprite(
				t[GFX::SPARKS_LILBLOOD], _p.x, _p.y, 0.75, 0.75, 0, 0, degtorad(l * 10), rgba(0xffffff, (l * 2) + 65));
			break;
		case 5:
		{
			_scala.x = iif(l > 10, 0.33 + (static_cast<float>(10) / l), 1.0);
			_scala.y = _scala.x;
			gfxdrawsprite(t[GFX::SPARKS_BLOOD],
				_p.x,
				_p.y,
				_scala.x,
				_scala.y,
		...
		case 6:
			...
			{
				gfxdrawsprite(t[sprite_system.GetSprite(owner).player->headcap],
					_p.x,
					_p.y,
					0,
					0,
					degtorad(l * 2),
					rgba(sprite_system.GetSprite(owner).player->shirtcolor));
			}
			break;
		case 7:
			...
			break;
		case 8:
			gfxdrawsprite(t[GFX::WEAPONS_SHELL], _p.x, _p.y, 1.1, 1.2, 0, 0, degtorad(l * 3.5), rgba(0xff3333));
			break;
		case 9:
		...
		break;
		case 25:
			gfxdrawsprite(t[GFX::SPARKS_SPAWNSPARK],
				_p.x - 20,
				_p.y - 20,
				0,
				0,
		...
		case 29:
		{
			...
			_p.x = _p.x - (22 * _scala.x);
			_p.y = _p.y - 48 + (l / 1.5);
			gfxdrawsprite(t[GFX::SPARKS_BIGSMOKE], _p.x, _p.y, _scala.x, _scala.y, rgba(0xffffff, trunc(2.5 * l)));
		}
		break;
		case 30:
		...
		case 36:
		{
			...
			gfxdrawsprite(
				t[GFX::SPARKS_PLOMYK], _p.x, _p.y, _scala.x, _scala.y, rgba(0xffffff, min((l * 2) + 185.0, 255.0)));
		}
		break;
		case 37:
		{
		...
		case 40:
			gfxdrawsprite(
				t[GFX::SPARKS_ODLAMEK1], _p.x, _p.y, 0, 0, degtorad(l * 8), rgba(0xffffff, std::trunc(l + 10)));
			break;
		case 41:
			gfxdrawsprite(
				t[GFX::SPARKS_ODLAMEK2], _p.x, _p.y, 0, 0, degtorad(l * 8), rgba(0xffffff, std::trunc(l + 10)));
			break;
		case 42:
			gfxdrawsprite(
				t[GFX::SPARKS_ODLAMEK3], _p.x, _p.y, 0, 0, degtorad(l * 8), rgba(0xffffff, std::trunc(l + 10)));
			break;
		case 43:
			gfxdrawsprite(
				t[GFX::SPARKS_ODLAMEK4], _p.x, _p.y, 0, 0, degtorad(l * 8), rgba(0xffffff, std::trunc(l + 10)));
			break;
		case 44:
		...
		case 47:
			...
			break;
		case 48:
			gfxdrawsprite(t[GFX::SPARKS_SKRAWEK],
				_p.x,
				_p.y,
				0,
				0,
				degtorad(l * 5),
				rgba(sprite_system.GetSprite(owner).player->shirtcolor, std::trunc(l * 2) + 15));
			break;
		case 49:
			gfxdrawsprite(t[GFX::SPARKS_SKRAWEK],
				_p.x,
				...
				0,
				degtorad(l * 5),
				rgba(sprite_system.GetSprite(owner).player->pantscolor, std::trunc(l * 2) + 15));
			break;
		case 50:
		...
		case 55:
		{
			...
			_scala.y = _scala.x;
			gfxdrawsprite(t[GFX::SPARKS_SPLAT],
				_p.x,
				_p.y,
				_scala.x,
				_scala.y,
		...
		case 60:
		{
			...
			_p.y = _p.y - 30;
			gfxdrawsprite(t[GFX::SPARKS_BIGSMOKE], _p.x, _p.y, _scala.x, _scala.y, rgba(0xffffff, trunc(l / 3.3)));

			if (l > 30)
			{
				gfxdrawsprite(t[GFX::SPARKS_BIGSMOKE2],
					_p.x,
					_p.y,
					_scala.x,
					_scala.y,
					rgba(0x666666, std::trunc(((255 - l)) / 9)));
			}
			else
			{
				gfxdrawsprite(t[GFX::SPARKS_BIGSMOKE2], _p.x, _p.y, _scala.x, _scala.y, rgba(0xbfbfbf, trunc(l * 1.0)));
			}
		}
		break;
		case 61:
			...
			break;
		case 62:
			gfxdrawsprite(t[GFX::SPARKS_JETFIRE],
				_p.x,
				_p.y,
				0,
				0,
		...
		case 64:
		{
			...
			_scala.y = l / 35;
			_p.y = _p.y - (static_cast<float>(1) / _scala.y);
			gfxdrawsprite(t[GFX::SPARKS_PLOMYK], _p.x, _p.y, _scala.x, _scala.y, rgba(0xffffff, (l * 2) + 185));
		}
		break;
		case 65:
					...
					if (map.pointinpolyedges(pos.x, pos.y, w.Index))
					{
						...
						switch (style)
						{
							...
							case 62:
							{
								...
								if (Random(2) == 0)
								{
									...
									gGlobalStateSound.playsound(
										SfxEffect::ts, gGlobalStateSparks.GetSparkParts().pos[num]);
								}
							}
							break;
							case 33:
							...
							case 6:
							{
								...
								{
									gGlobalStateSound.playsound(
										SfxEffect::clipfall, gGlobalStateSparks.GetSparkParts().pos[num]);
								}
							...
							case 73:
							{
								...
								{
									gGlobalStateSound.playsound(
										SfxEffect::shell + Random(2), gGlobalStateSparks.GetSparkParts().pos[num]);
								}
								if (collidecount > 4)
								{
									kill();
							...
							{
								gGlobalStateSound.playsound(
									SfxEffect::gaugeshell, gGlobalStateSparks.GetSparkParts().pos[num]);
								if (collidecount > 4)
								{
									kill();
								}
