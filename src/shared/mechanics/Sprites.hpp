...
struct tcontrol
{
	bool left, right, up, down, fire, jetpack, thrownade, changeweapon, throwweapon, reload, prone, flagthrow;
...
public:
	...
	void update();
	void kill();
	void die(std::int32_t how, std::int32_t who, std::int32_t where, std::int32_t what, tvector2 impact);
	...
	void applyweaponbynum(std::uint8_t wnum,
		std::uint8_t gun,
		std::int32_t ammo = -1,
		bool restoreprimarystate = false); // Ammo -1
	void healthhit(float amount, std::int32_t who, std::int32_t where, std::int32_t what, const tvector2& impact);
	void freecontrols();
...
public:
	...
	static constexpr SpriteId Invalid() { return SpriteId{ 255 }; }

	constexpr bool operator==(const SpriteId& ref) const { return ref.Id == Id; }

private:
	const std::uint8_t Id;

	[[nodiscard]] auto GetId() const -> std::remove_const_t<decltype(Id)> { return Id; }
	friend class TSpriteSystem<>;
};

template <Config::Module M = Config::GetModule()>
std::int32_t createsprite(
	tvector2& spos, SpriteId id, std::shared_ptr<tplayer> player, const tsprite::Style style = tsprite::Style::Default);
