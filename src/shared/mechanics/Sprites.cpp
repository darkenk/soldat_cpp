...
auto createsprite(
	tvector2& spos, const SpriteId id, const std::shared_ptr<tplayer> player, const tsprite::Style /*style*/)
	-> std::int32_t // NOLINT
{
	...
	if ((secwep >= 1) && (secwep <= secondary_weapons) && (weapon.IsEnabled(primary_weapons + secwep)))
	{
...
auto teamcollides(PolygonType polytype, std::int32_t team, const bool bullet) -> bool
{
	...
	else
	{
		if (((polytype == poly_type_red_bullets) && (team == team_alpha))
			|| (((polytype == poly_type_red_bullets) || (polytype == poly_type_red_player)) && (team != team_alpha)))
		{
			result = false;
		}
...
void Sprite<M>::update()
{
	...
	if (autoreloadwhencanfire && ((weapon.num != spas12_num) || (weapon.fireintervalcount == 0)))
	{
		...
		if ((weapon.num == spas12_num) && (bodyanimation.id != AnimationType::Roll)
			&& (bodyanimation.id != AnimationType::RollBack) && (bodyanimation.id != AnimationType::Change)
	...
	for (i = 1; i <= 20; i++)
	{
		if (skeleton.active[i] && !deadmeat)
		{
			...
			{
				// legs
				if ((i == 1) || (i == 4) || (i == 2) || (i == 3) || (i == 5) || (i == 6) || (i == 17) || (i == 18))
				{
					skeleton.pos[i].x =
						spritePartsPos.x + (direction * legsanimation.frames[legsanimation.currframe].pos[i].x);
					skeleton.pos[i].y = spritePartsPos.y + legsanimation.frames[legsanimation.currframe].pos[i].y;
				}
			}
				...
				|| (i == 15) || (i == 16) || (i == 19) || (i == 20))
			{
				...
				{
					skeleton.pos[i].y = 9 + spritePartsPos.y + bodyanimation.frames[bodyanimation.currframe].pos[i].y;
				}
			}
		}
	}
	...
	if (!deadmeat)
	{
		...
		// arm
		i = 15;
		if ((bodyanimation.id != AnimationType::Reload) && (bodyanimation.id != AnimationType::ReloadBow)
			&& (bodyanimation.id != AnimationType::ClipIn) && (bodyanimation.id != AnimationType::ClipOut)
			&& (bodyanimation.id != AnimationType::SlideBack) && (bodyanimation.id != AnimationType::Change)
			&& (bodyanimation.id != AnimationType::ThrowWeapon) && (bodyanimation.id != AnimationType::WeaponNone)
			&& (bodyanimation.id != AnimationType::Punch) && (bodyanimation.id != AnimationType::Roll)
			&& (bodyanimation.id != AnimationType::RollBack) && (bodyanimation.id != AnimationType::Cigar)
			&& (bodyanimation.id != AnimationType::Match) && (bodyanimation.id != AnimationType::Smoke)
			&& (bodyanimation.id != AnimationType::Wipe) && (bodyanimation.id != AnimationType::TakeOff)
...

	{
		...
		for (i = 1; i <= 20; i++)
		{
			...
			if (deadmeat || (halfdead && isnotspectator()))
			{
				...
				{
					if (!skeleton.constraints[k].active
						and ((skeleton.constraints[k].parta == i) || (skeleton.constraints[k].partb == i)))
					{
						m4 = skeleton.pos[i];
						m4.y = m4.y + 2;
	...
	if (!deadmeat)
	{
		...
		switch (style)
		{
			case tsprite::Style::Default:
			{
				...
				onground = onground || checkmapcollision(spritePartsPos.x - 2, spritePartsPos.y + 2 - arms, 0);

				// radius collison check
				ongroundforlaw = checkradiusmapcollision(spritePartsPos.x, spritePartsPos.y - 1, onground);

				onground = checkmapverticescollision(spritePartsPos.x, spritePartsPos.y, 3, onground || ongroundforlaw)
						|| onground;
				...
				// WEAPON HANDLING
#ifndef SERVER
				if ((sprite_system.IsPlayerSprite(num)) || (weapon.fireinterval <= fireinterval_net)
					or !GS::GetGame().pointvisible(
						spritePartsPos.x, spritePartsPos.y, gGlobalStateClient.camerafollowsprite))
#endif
				{
					if ((weapon.fireintervalcount > 0) && ((weapon.ammocount > 0) || (weapon.num == spas12_num)))
					{
						weapon.fireintervalprev = weapon.fireintervalcount;
				...
				if ((weapon.ammocount == 0)
					&& ((weapon.num == chainsaw_num)
						|| ((bodyanimation.id != AnimationType::Roll) && (bodyanimation.id != AnimationType::RollBack)
							&& (bodyanimation.id != AnimationType::Melee) && (bodyanimation.id != AnimationType::Change)
				...
				if ((weapon.num == chainsaw_num) && (stat == 0))
				{
					if (CVar::r_maxsparks > (max_sparks - 10))
					{
						if (GS::GetGame().GetMainTickCounter() % 15 == 0)
						{
							...
							if (weapon.ammocount == 0)
							{
								gGlobalStateSound.playsound(
									SfxEffect::chainsaw_o, spritePartsPos, gattlingsoundchannel);
							}
							else
							{
								gGlobalStateSound.playsound(
									SfxEffect::chainsaw_m, spritePartsPos, gGlobalStateSound.defaultchannel);
							}
						}
					}
				...
				if (para == 1)
				{
					...
					{
						ceasefire =
							(CVar::sv_survivalmode && (ceasefirecounter < GS::GetGame().GetCeasefiretime() * 3 - 30))
	...
	if (k == 1)
	{
		for (j = primary_weapons + 1; j <= main_weapons; j++)
		{
			if (weaponSystem.IsEnabled(j))
			{
				...
				if (gGlobalStateGameMenus.limbomenu->active && !sprite_system.GetSprite(sprite_id).deadmeat)
				{
					sprite_system.GetSprite(sprite_id).applyweaponbynum(GS::GetWeaponSystem().GetGuns()[j].num, 2);
				}
				clientspritesnapshot();
				break;
			}
...
template <Config::Module M>
void Sprite<M>::die(std::int32_t how, std::int32_t who, std::int32_t where, std::int32_t what, tvector2 impact)
{
	auto& sprite_system = SpriteSystem::Get();
	std::int32_t i = 0;
	std::int32_t j = 0;
	...
	if (!deadmeat)
	{
		...
		if (CVar::sv_bullettime)
		{
			if (GS::GetGame().IsDefaultGoalTicks())
			{
				...
				for (auto& sprite : sprite_system.GetActiveSprites())
				{
					...
					if (sprite.active && (std::cmp_not_equal(sprite.num, who)) && (!sprite.player->demoplayer)
						and sprite.isnotspectator())
					{
		...
		if (what > 0)
		{
			if (((bullet[what].style == bullet_style_arrow) && (CVar::sv_gamemode != gamestyle_rambo))
				|| ((bullet[what].style == bullet_style_flamearrow) && (CVar::sv_gamemode != gamestyle_rambo)))
			{
				if (!bullet[what].dontcheat)
				{
					gGlobalStateServer.kickplayer(
						sprite_system.GetSprite(who).num, true, kick_cheat, day, "Not allowed weapon");
					return;
				}
			}
		}
		...
		if (CVar::sv_punishtk)
		{
			if (isinsameteam(sprite_system.GetSprite(who)) && !(CVar::sv_gamemode == gamestyle_deathmatch)
				...
				&& !(player->name == sprite_system.GetSprite(who).player->name))
			{
				...
				GS::GetMainConsole().Console(sprite_system.GetSprite(who).player->name + " Team Killed " + player->name
												 + " (Warning #"
		...
		{
			GS::GetMainConsole().Console(std::string("(") + inttostr(sprite_system.GetSprite(who).player->team) + ") "
...
#ifdef SERVER
		{
			...
			if (CVar::bots_chat)
			{
				...
				if ((std::cmp_not_equal(who, num)) && (sprite_system.GetSprite(who).player->controlmethod == bot))
				{
					if (Random(sprite_system.GetSprite(who).brain.chatfreq / 3) == 0)
					{
						serversendstringmessage(
							(sprite_system.GetSprite(who).brain.chatkill), all_players, who, msgtype_pub);
					}
				}
			}
...
#ifndef SERVER
		if (((sprite_system.IsPlayerSprite(who)) || (sprite_system.IsPlayerSprite(num))) && (what > 0)
	...
	if (what > 0)
	{
		...
		else
		{
			...
			gGlobalStateNetworkServerSprite.shotlifeServer =
				(float)((GS::GetGame().GetMainTickCounter() - b.startuptime)) / 60;
		}
	}
					...
					if (Random(100) > 50)
					{
						...
						for (i = 0; i <= 50; i++)
						{
							...
							gGlobalStateSparks.createspark(
								a, b, iif(i < 25, randomrange(4, 5), 5), num, 100 - Random(20));
						}
					}
...

				{
					...
					if (show_log)
					{
						GS::GetMainConsole().Console(
							_("Players left:") + ' ' + (inttostr(alivenum)), game_message_color);
					}
				}
			}
	...
	for (i = 1; i <= max_things; i++)
	{
		if (things[i].holdingsprite == num)
		{
			if (things[i].style < object_ussocom)
			{
				...
				{
					GS::GetMainConsole().Console(wideformat(_("{} dropped the {} Flag"),
													 player->name,
					...
					if (isinsameteam(sprite_system.GetPlayerSprite()))
					{
						...
						{ // Alpha
							gGlobalStateClientGame.bigmessage(
								wideformat(_("{} Flag dropped!"), _("Red")), capturemessagewait, capture_message_color);
						}
...
#ifndef SERVER
			if ((std::cmp_not_equal(num, who)) && (isnotinsameteam(sprite_system.GetSprite(who)) or issolo()))
#endif
			{
...
auto Sprite<M>::dropweapon() -> std::int32_t
{
	...
	if (result > 0)
	{
		...
		scrptdispatcher.onweaponchange(
			num, noweapon_num, secondaryweapon.num, guns[noweapon].ammocount, secondaryweapon.ammocount);
	}
		...
		for (const auto& w : sector.GetPolys())
		{
				...
				|| ((holdedthing == 0) && (polytype == poly_type_not_flaggers)))
			{
				if (Polymap::PointInPoly(pos, w))
				{
...
#ifndef SERVER
					// Run
					if (((legsanimation.id == AnimationType::Run) || (legsanimation.id == AnimationType::RunBack))
					...
					if ((area == 0)
						|| ((area == 1)
							&& ((spriteVelocity.y < 0) || (spriteVelocity.x > slidelimit)
					...
					if (area == 0)
					{
						if ((legsanimation.id == AnimationType::Stand) || (legsanimation.id == AnimationType::Crouch)
							|| (legsanimation.id == AnimationType::Prone)
							|| (legsanimation.id == AnimationType::ProneMove)
							|| (legsanimation.id == AnimationType::GetUp) || (legsanimation.id == AnimationType::Fall)
							|| (legsanimation.id == AnimationType::Mercy) || (legsanimation.id == AnimationType::Mercy2)
...
void BackgroundState<M>::backgroundtestbigpolycenter(const tvector2& pos)
{
	...
	if (backgroundstatus == background_transition)
	{
		...
		else if ((backgroundpoly != background_poly_none) and (Polymap::pointinpoly(pos, map.polys[backgroundpoly])))
		{
			backgroundtestresult = true;
		}
	}
...
template <Config::Module M>
void Sprite<M>::applyweaponbynum(std::uint8_t wnum, std::uint8_t gun, std::int32_t ammo, bool restoreprimarystate)
{
...
template <Config::Module M>
void Sprite<M>::healthhit(float amount, std::int32_t who, std::int32_t where, std::int32_t what, const tvector2& impact)
{
	auto& sprite_system = SpriteSystem::Get();
	tvector2 t;
	float hm = NAN;
...
#ifdef SERVER
	if (sprite_system.GetSprite(who).isspectator() && (sprite_system.GetSprite(who).player->controlmethod == human))
	{
		return;
	}
#endif
	...
	if (!this->deadmeat && (what > 0))
	{
		...
		if ((std::cmp_not_equal(who, this->num)) && (!bullet[what].hashit) && (sprite_system.IsPlayerSprite(who)))
		{
			for (j = 1; j <= 20; j++)
			{
	...
	// helmet fall off
	if ((Health < helmetfallhealth) && (wearhelmet == 1) && (where == 12) && (weapon.num != bow_num)
		&& (weapon.num != bow2_num) && (player->headcap > 0))
	{
		wearhelmet = 0;
#ifndef SERVER
		auto& spriteVelocity = sprite_system.GetVelocity(num);
...
void Sprite<M>::respawn()
{
	...
	if ((secwep >= 1) && (secwep <= secondary_weapons) && (weaponSystem.IsEnabled(primary_weapons + secwep))
	...
	if (player->controlmethod == bot)
	{
		...
		if ((brain.favweapon != noweapon_num) && (brain.favweapon != knife_num) && (brain.favweapon != chainsaw_num)
			&& (brain.favweapon != law_num) && !dummy)
		{
		...
		if ((!weaponSystem.IsEnabled(1)) && (!weaponSystem.IsEnabled(2)) && (!weaponSystem.IsEnabled(3))
			&& (!weaponSystem.IsEnabled(4)) && (!weaponSystem.IsEnabled(5)) && (!weaponSystem.IsEnabled(6))
			&& (!weaponSystem.IsEnabled(7)) && (!weaponSystem.IsEnabled(8)) && (!weaponSystem.IsEnabled(9))
			&& (!weaponSystem.IsEnabled(10)) && (weaponsel[num][1] == 0) && (weaponsel[num][2] == 0)
...
void Sprite<M>::changeteam_ServerVariant(std::int32_t team, bool adminchange, std::uint8_t jointype) // NOLINT
{
	...
	if (active)
	{
		...
		if ((teamscount[team_spectator] >= CVar::sv_maxspectators) && (team == team_spectator) && (!adminchange))
		{
			writeconsole(num, "Spectators are full", rgb(0, 0, 255));
			return;
		}

		if ((CVar::sv_gamemode != gamestyle_teammatch) && ((team == team_charlie) || (team == team_delta))
			&& (!adminchange))
		{
			return;
		}
		...
		switch (player->team)
		{
			...
			case team_none:
				GS::GetMainConsole().Console(
					wideformat(_("%s has joined the game."), player->name), enter_message_color);
				break;
			case team_alpha:
				GS::GetMainConsole().Console(
					wideformat(_("%s has joined alpha team"), player->name), alphaj_message_color);
				break;
			case team_bravo:
				GS::GetMainConsole().Console(
					wideformat(_("%s has joined bravo team"), player->name), bravoj_message_color);
				break;
			case team_charlie:
				GS::GetMainConsole().Console(
					wideformat(_("%s has joined charlie team"), player->name), charliej_message_color);
				break;
			case team_delta:
				GS::GetMainConsole().Console(
					wideformat(_("%s has joined delta team"), player->name), deltaj_message_color);
				break;
#endif
		}
...
#ifdef SERVER
		if ((GS::GetGame().GetMapchangecounter() > -60) && (GS::GetGame().GetMapchangecounter() < 99999999))
		{
			servermapchange(num);
		}
#endif
...
void Sprite<M>::fire()
{
	...
	// Bullet spread
	if ((weapon.num != eagle_num) && (weapon.num != spas12_num) && (weapon.bulletstyle != bullet_style_shotgun))
	{
	...
	{ // LAW
		if ((onground || ongroundpermanent || ongroundforlaw)
			&& (((legsanimation.id == AnimationType::Crouch) && (legsanimation.currframe > 13))
				|| (legsanimation.id == AnimationType::CrouchRun) || (legsanimation.id == AnimationType::CrouchRunBack)
				|| ((legsanimation.id == AnimationType::Prone) && (legsanimation.currframe > 23))))
		{
			bn = createbullet(a, b, weapon.num, num, 255, weapon.hitmultiply, true, false);
		}
	...
	if (sprite_system.IsPlayerSprite(num))
	{
		...
		if (weapon.bink < 0)
		{
			if ((legsanimation.id == AnimationType::Crouch) || (legsanimation.id == AnimationType::CrouchRun)
				|| (legsanimation.id == AnimationType::CrouchRunBack) || (legsanimation.id == AnimationType::Prone)
				|| (legsanimation.id == AnimationType::ProneMove))
			{
			...
			{
				gGlobalStateClient.hitspraycounter = calculatebink(gGlobalStateClient.hitspraycounter, -weapon.bink);
			}
		}
	}
	...
	if (((sprite_system.IsPlayerSpriteValid()) && (gGlobalStateClient.camerafollowsprite != 0))
		and ((sprite_system.IsPlayerSprite(num)) || CVar::cl_screenshake))
	{
		if (weapon.num != chainsaw_num)
		{
			if (GS::GetGame().pointvisible(spritePartsPos.x, spritePartsPos.y, gGlobalStateClient.camerafollowsprite))
			{
	...
	if (sprite_system.IsPlayerSprite(num))
	{
		...
		if (rc > 0)
		{
			...
			calculaterecoil(gGlobalStateGame.gamewidthhalf - gGlobalStateClient.camerax + spritePartsPos.x,
				...
				for (i = 1; i <= max_things; i++)
				{
					...
					if (thing.holdingsprite == num)
					{
						if (thing.style < 4)
						{
							...
							if (!map.raycast(skeleton.pos[15], lookpoint1, d, 200, false, true, false)
								and !map.raycast(skeleton.pos[15], lookpoint2, d, 200, false, true, false)
								and !map.raycast(skeleton.pos[15], lookpoint3, d, 200, false, true, false)
								and !map.collisiontest(futurepoint1, bperp, true)
								and !map.collisiontest(futurepoint2, bperp, true)
								and !map.collisiontest(futurepoint3, bperp, true)
								and !map.collisiontest(futurepoint4, bperp, true))
							{
								for (j = 1; j <= 4; j++)
								{
...
void Sprite<M>::throwgrenade()
{
...
#ifndef SERVER
	// Pull pin
	if ((bodyanimation.id == AnimationType::Throw) && (bodyanimation.currframe == 15) && (tertiaryweapon.ammocount > 0)
		&& (ceasefirecounter < 0))
	{
	...
	if ((bodyanimation.id == AnimationType::Throw) && (!control.thrownade || (bodyanimation.currframe == 36)))
	{
		// Grenade throw
		if ((bodyanimation.currframe > 14) && (bodyanimation.currframe < 37) && (tertiaryweapon.ammocount > 0)
			&& (ceasefirecounter < 0))
		{
			...
			if (!map.collisiontest(a, c) and !map.raycast(e, a, f, 50, false, false, true, false, player->team))
			{
				createbullet(a, b, tertiaryweapon.num, num, 255, guns[fraggrenade].hitmultiply, true, false);
				...
				bool const is_current_sprite = [&]()
				{
					...
					{
						return (
							(player->controlmethod == human) && (TSpriteSystem<Sprite<M>>::Get().IsPlayerSprite(num)));
					}
					return false;
				}();
	...
	if (moveacc > 0)
	{
			...
			|| (legsanimation.id == AnimationType::RollBack))
		{
			result = moveacc * 7;
					 ...
					 && (legsanimation.id != AnimationType::ProneMove) && (legsanimation.id != AnimationType::Crouch)
					 && (legsanimation.id != AnimationType::CrouchRun)
					 && (legsanimation.id != AnimationType::CrouchRunBack))
				 || (legsanimation.id == AnimationType::GetUp)
				 || ((legsanimation.id == AnimationType::Prone) && (legsanimation.currframe < legsanimation.numframes)))
		{
			result = moveacc * 3;
		}
	}
...
{
	bool result = false;
	result = (CVar::sv_survivalmode == false) or (GS::GetGame().GetSurvivalEndRound()) or (!deadmeatbeforerespawn);
	return result;
}
