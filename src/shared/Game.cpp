...
void Game<M>::number27timing()
{
	...
	timepassed = timepassed + std::chrono::duration_cast<std::chrono::milliseconds>(timeinmil - timeinmillast);
	secondslast = seconds;
...
auto Game<M>::pointvisible(float x, float y, const std::int32_t i) -> bool
{
	...
	sx = spritePartsPos.x
	   - (static_cast<float>((spritePartsPos.x - SpriteSystem::Get().GetSprite(i).control.mouseaimx)) / 2);
	sy = spritePartsPos.y
	   - (static_cast<float>((spritePartsPos.y - SpriteSystem::Get().GetSprite(i).control.mouseaimy)) / 2);

	if ((x > (sx - game_width)) && (x < (sx + game_width)) && (y > (sy - game_height)) && (y < (sy + game_height)))
	{
		result = true;
	}
	return result;
...
template <Config::Module M>
void Game<M>::startvote(std::uint8_t startervote, std::uint8_t typevote, std::string targetvote, std::string reasonvote)
{
	auto& sprite_system = SpriteSystem::Get();
	VoteActive = true;
...
void Game<M>::changemap()
{
	...
	else
	{
		...
		if ((gGlobalStateClient.camerafollowsprite == 0)
			or !sprite_system.GetSprite(gGlobalStateClient.camerafollowsprite).IsActive())
		{
...
void Game<M>::sortplayers()
{
	...
	gGlobalStateGame.sortedteamscore[1].color = (std::uint32_t(CVar::ui_status_transparency) << 24) | 0xd20f05; // ARGB
	gGlobalStateGame.sortedteamscore[2].color = (std::uint32_t(CVar::ui_status_transparency) << 24) | 0x50fd2;
	gGlobalStateGame.sortedteamscore[3].color = (std::uint32_t(CVar::ui_status_transparency) << 24) | 0xd2d205;
	gGlobalStateGame.sortedteamscore[4].color = (std::uint32_t(CVar::ui_status_transparency) << 24) | 0x5d205;
...
void Game<M>::CalculateTeamAliveNum(int32_t player)
{
	...
	if (((teamalivenum[1] > 0) && (teamalivenum[2] < 1) && (teamalivenum[3] < 1) && (teamalivenum[4] < 1))
		|| ((teamalivenum[2] > 0) && (teamalivenum[1] < 1) && (teamalivenum[3] < 1) && (teamalivenum[4] < 1))
		|| ((teamalivenum[3] > 0) && (teamalivenum[1] < 1) && (teamalivenum[2] < 1) && (teamalivenum[4] < 1))
		|| ((teamalivenum[4] > 0) && (teamalivenum[1] < 1) && (teamalivenum[2] < 1) && (teamalivenum[3] < 1))
		|| ((teamalivenum[1] < 1) && (teamalivenum[2] < 1) && (teamalivenum[3] < 1) && (teamalivenum[4] < 1)))
	{
		auto& activeSprites = sprite_system.GetActiveSprites();
		std::for_each(std::begin(activeSprites),
			std::end(activeSprites),
			[](auto& sprite)
			{
