...
public:
	...
	void startvote(std::uint8_t startervote, std::uint8_t typevote, std::string targetvote, std::string reasonvote);
	...
	std::uint8_t GetAlivenum() const { return alivenum; }
	void SetAlivenum(std::uint8_t _Alivenum) { alivenum = _Alivenum; }

	std::int8_t GetTeamAliveNum(std::int32_t idx) const { return teamalivenum[idx]; }

	bool GetSurvivalEndRound() const { return survivalendround; }
	void SetSurvivalendround(bool _survivalendround) { survivalendround = _survivalendround; }

	std::int8_t GetTeamplayersnum(std::int32_t idx) const { return teamplayersnum[idx]; }

	void CalculateTeamAliveNum(std::int32_t player);

	std::int32_t GetCeasefiretime() const { return ceasefiretime; }

	std::int32_t GetMapchangecounter() const { return mapchangecounter; }

	void SetMapchangecounter(std::int32_t _mapchangecounter) { mapchangecounter = _mapchangecounter; }

	std::int32_t GetStarthealth() const { return starthealth; }
	void SetStarthealth(std::int32_t _starthealth) { starthealth = _starthealth; }

	const std::string& GetMapchangename() const { return mapchangename; }

	void SetMapchangename(const std::string& _mapchangename) { mapchangename = _mapchangename; }

	std::int32_t GetTimelimitcounter() const { return timelimitcounter; }
	void SetTimelimitcounter(std::int32_t _timelimitcounter) { timelimitcounter = _timelimitcounter; }

	std::int32_t GetTimeleftmin() const { return timeleftmin; }
	void SetTimeleftmin(std::int32_t _timeleftmin) { timeleftmin = _timeleftmin; }

	std::int32_t GetTimeleftsec() const { return timeleftsec; }
	void SetTimeleftsec(std::int32_t _timeleftsec) { timeleftsec = _timeleftsec; }

	void SetMapchange(const tmapinfo& _mapchange) { mapchange = _mapchange; }

	const tmapinfo& GetMapchange() const { return mapchange; }

	void SetMapchangechecksum(const tsha1digest& _mapchangechecksum) { mapchangechecksum = _mapchangechecksum; }

	std::int32_t GetMapchangetime() const { return mapchangetime; }

	std::int32_t GetMainTickCounter() const { return maintickcounter; }

	void TickMainTickCounter() { maintickcounter += 1; }

	void ResetMainTickCounter() { maintickcounter = 0; }

	std::int32_t GetPlayersNum() const { return playersnum; }

	void SetPlayersNum(std::int32_t num) { playersnum = num; }

	std::int32_t GetBotsNum() const { return botsnum; }

	std::int32_t GetSpectatorsNum() const { return spectatorsnum; }

	std::int32_t GetPlayersTeamNum(std::int32_t idx) const { return playersteamnum[idx]; }

	std::int32_t GetTeamScore(std::int32_t idx) const { return teamscore[idx]; }

	void SetTeamScore(std::int32_t idx, std::int32_t value) { teamscore[idx] = value; }

	std::int32_t GetTeamFlag(std::int32_t idx) const { return teamflag[idx]; }

	void SetTeamFlag(std::int32_t idx, std::int32_t value) { teamflag[idx] = value; }

	float GetSinusCounter() const { return sinuscounter; }
	void SetSinusCounter(float _sinuscounter) { sinuscounter = _sinuscounter; }

	const tsha1digest& GetCustomModChecksum() const { return custommodchecksum; }
	void SetCustomModChecksum(const tsha1digest& _custommodchecksum) { custommodchecksum = _custommodchecksum; }

	const tsha1digest& GetGameModChecksum() const { return gamemodchecksum; }
	void SetGameModChecksum(const tsha1digest& _gamemodchecksum) { gamemodchecksum = _gamemodchecksum; }

	const tsha1digest& GetMapChecksum() const { return mapchecksum; }
	void SetMapChecksum(const tsha1digest& _mapchecksum) { mapchecksum = _mapchecksum; }

	const tkillsort& GetSortedPlayers(std::int32_t idx) const { return sortedplayers[idx]; }
