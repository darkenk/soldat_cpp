...
{
public:
	CVarBase(const std::string_view& name,
		const std::string_view& description,
		FlagSet<CVarFlags> flags,
		const T& value,
		const std::uint8_t id = 255)
	...
	~CVarBase() { sGetCVars().erase(Name); }

	bool operator==(const T& ref) const { return ref == Value; }
	...
	bool ParseAndSetValue(const std::string_view& value) noexcept { return FromString<T>(value, Value); }
	...
	{
		auto& CVars = sGetCVars();
		auto pred = [id](const auto& cv) -> bool
		{
			return cv.second->Id == id;
		};
	...
	[[nodiscard]] bool IsValid() const { return this != &sGetInvalidCVar(); }

	[[nodiscard]] bool IsSyncable() const { return Flags.Test(CVarFlags::SYNC); }
	...
	public:
		...
		public:
			...
			bool operator==(iterator other) const { return InternalIter == other.InternalIter; }
			bool operator!=(iterator other) const { return !(*this == other); }
			CVarBase& operator*() const { return *(InternalIter->second); }
		};
		iterator begin() { return iterator(CVarBase::sGetCVars().begin()); }
		iterator end() { return iterator(CVarBase::sGetCVars().end()); }
	};

	static AllCVars GetAllCVars() { return AllCVars(); }

	[[nodiscard]] std::uint8_t GetId() const { return Id; }
