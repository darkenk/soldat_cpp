...
public:
	...
	BitStream() { Buffer.reserve(INITIAL_BUFFER_SIZE); }

	~BitStream() { Buffer.clear(); }

	const BitBuffer& Data() { return Buffer; }
...
private:
	...
	{
		constexpr std::array<std::uint8_t, 8> mask = { 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80 };
