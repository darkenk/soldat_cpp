...
public:
	...
	void Set(const T& flag) { FlagStorage |= static_cast<EnumStorageType>(flag); }

	bool Test(const T& flag) const { return FlagStorage & static_cast<EnumStorageType>(flag); }
...
template <typename T>
FlagSet<typename std::enable_if<EnableFlagSet<T>::enable, T>::type> operator|(const T& left, const T& right)
{
