...
RectangleArea::RectangleArea(std::uint32_t width, std::uint32_t height) : Width(width), Height(height)
{
...
auto RectangleArea::Add(Rectangle& rect) -> bool
{
	...
	do
	{
		...
		it = std::ranges::min_element(Skyline,
			[fit](auto const& l, auto const& r)
			{
				auto fitL = fit(l);
				auto fitR = fit(r);
				if (fitL && fitR)
...
void RectangleArea::MergeItems(SkylineItemContainer::iterator mergeTo, SkylineItemContainer::iterator mergeFrom)
{
...
{
	auto it = std::ranges::min_element(Skyline,
