...
namespace
{
	...
	void ReadConf(
		const TIniFile::Entries& conf, const std::string_view entry, std::string& out, bool allowEmpty = false)
	{
	...
	void ReadConf(const TIniFile::Entries& conf, const std::string_view entry, bool& out, bool allowEmpty = false)
	{
		std::string s;
	...
	void ReadConf(const TIniFile::Entries& conf, const std::string_view entry, float& out, bool allowEmpty = false)
	{
		std::string s;
		ReadConf(conf, entry, s, allowEmpty);
		out = std::stof(s);
	...
	template <typename T>
	void ReadConf(const TIniFile::Entries& conf, const std::string_view entry, T& out, bool allowEmpty = false)
	...
	void ReadConfColor(
		const TIniFile::Entries& conf, const std::string_view entry, std::uint32_t& out, bool allowEmpty = false)
	{
		std::string s;
		ReadConf(conf, entry, s, allowEmpty);
		SoldatAssert(s[0] == '$');
...
auto loadweaponsconfig(TIniFile& iniFile, std::string& modname, std::string& modversion, GunsDescription& gunDesc)
	-> bool
{
	TIniFile::Entries conf;
