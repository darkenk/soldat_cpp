...
void serverbulletsnapshot(const std::uint8_t i, std::uint8_t tonum, bool forced)
{
	...
	if (!forced)
	{
		if ((sprite_system.GetSprite(bulletmsg.owner).weapon.ammocount > 0) && (b.style != bullet_style_fragnade)
	...
	for (auto& sprite : sprite_system.GetActiveSprites())
	{
		if ((sprite.player->controlmethod == human) && (sprite.num != b.owner))
		{
			...
			{
				if (bulletcansend(GetBulletParts().pos[i].x,
						GetBulletParts().pos[i].y,
						sprite.player->camera,
						GetBulletParts().velocity[i].x)
					or forced)
				{
					gGlobalStateNetworkServer.GetServerNetwork()->SendData(
						&bulletmsg, sizeof(bulletmsg), sprite.player->peer, false);
				}
			}
		}
	}
	...
	tmsgheader* netmessage, std::int32_t size, NetworkServer& /*network*/, TServerPlayer* player)
{
	...
	sprite.bulletcheck[sprite.bulletcheckindex] = bulletsnap->seed;
		...
		for (i = 0; i <= max_things; i++)
		{
			if ((things[i].style == object_stationary_gun) /*Stat Gun*/ && (!onstatgun))
			{
				if (distance(sprite.skeleton.pos[1].x,
						sprite.skeleton.pos[1].y,
						things[i].skeleton.pos[1].x,
						things[i].skeleton.pos[1].y)
					< stat_radius * 2)
				{
					onstatgun = true;
				}
	...
	if ((style != bullet_style_fragnade) && (style != bullet_style_punch) && (style != bullet_style_clusternade)
		&& (style != bullet_style_cluster) && (style != bullet_style_thrownknife) && (style != bullet_style_m2)
	...
	if ((oldbulletsnapshotmsg[p].weaponnum == bulletsnap->weaponnum)
		&& (oldbulletsnapshotmsg[p].pos.x == bulletsnap->pos.x) && (oldbulletsnapshotmsg[p].pos.y == bulletsnap->pos.y)
	...
	if ((style != bullet_style_fragnade) && (style != bullet_style_clusternade) && (style != bullet_style_cluster)
		&& (style != bullet_style_thrownknife) && (style != bullet_style_m2))
	{
		if (vec2length(bulletsnap->velocity) > sprite.lastweaponspeed + 10 * guns[weaponindex].inheritedvelocity)
		{
			return;
		}
	}
	...
	if ((style != bullet_style_fragnade) && (style != bullet_style_flame) && (style != bullet_style_clusternade)
		&& (style != bullet_style_cluster) && (style != bullet_style_thrownknife) && (style != bullet_style_m2))
	{
		if (vec2length(b) > 366)
		{
	...
	if (style == bullet_style_thrownknife)
	{
		...
		{
			if ((!gGlobalStateNetworkServer.knifecan[p]) or (sprite.weapon.bulletstyle != bullet_style_thrownknife)
				or (sprite.weapon.bulletstyle != bullet_style_punch))
			{
