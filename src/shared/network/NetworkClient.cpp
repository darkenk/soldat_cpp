...
NetworkClientImpl::NetworkClientImpl() : mPeer(k_HSteamNetConnection_Invalid)
{
	...
	RegisterMsgHandler(msgid_clientspritesnapshot_dead, std::make_unique<clienthandleclientspritesnapshot_dead>());
	...
	RegisterMsgHandler(msgid_serverskeletonsnapshot, std::make_unique<clienthandleserverskeletonsnapshot>());
	RegisterMsgHandler(msgid_serverspritesnapshot, std::make_unique<clienthandleserverspritesnapshot>());
	RegisterMsgHandler(msgid_serverspritesnapshot_major, std::make_unique<clienthandleserverspritesnapshot_major>());
	RegisterMsgHandler(msgid_serversyncmsg, std::make_unique<clienthandleserversyncmsg>());
	RegisterMsgHandler(msgid_serverthingmustsnapshot, std::make_unique<clienthandleserverthingmustsnapshot>());
	RegisterMsgHandler(msgid_serverthingsnapshot, std::make_unique<clienthandleserverthingsnapshot>());
	...
	RegisterMsgHandler(msgid_weaponactivemessage, std::make_unique<clienthandleweaponactivemessage>());
}
...
auto NetworkClientImpl::SendDataImpl(
	const std::byte* data, std::int32_t size, bool reliable, const source_location& location) -> bool
{
	...
	LogDebug(NETMSG, "Senddata {} from {}", reinterpret_cast<const tmsgheader*>(data)->id, location.function_name());
...
template <Config::Module M>
auto GlobalStateNetworkClient::GetNetwork() -> NetworkClientImpl* requires(Config::IsClient(M)) { return mUdp.get(); }
