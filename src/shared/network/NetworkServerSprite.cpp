...
void GlobalStateNetworkServerSprite::serverspritesnapshot(std::uint8_t r)
{
	...
	for (auto& sprite : sprite_system.GetActiveSprites())
	{
		...
		{ // active player/sprite
			...
			sprite.vest = std::max<float>(sprite.vest, 0);
			...
			if (((vec2length(b) > veldelta)) || (GS::GetGame().GetMainTickCounter() - time_spritesnapshot_mov[i] > 30)
					...
					for (auto& sprite : sprite_system.GetActiveSprites())
					{
						if (sprite.player->controlmethod == human)
						{
							gGlobalStateNetworkServer.GetServerNetwork()->SendData(
								&servermsg, sizeof(servermsg), sprite.player->peer, false);
						}
					}
				}
			}
...
void GlobalStateNetworkServerSprite::serverspritesnapshotmajorfloat(const std::uint8_t who, std::uint8_t r)
{
	auto& sprite_system = SpriteSystem::Get();
	tmsg_serverspritesnapshot_major servermsg;
...
void GlobalStateNetworkServerSprite::serverspritedeath(
	std::int32_t who, std::int32_t killer, std::int32_t bulletnum, std::int32_t where) const
{
	auto& sprite_system = SpriteSystem::Get();
...
void GlobalStateNetworkServerSprite::serverspritedeltas(const std::uint8_t i)
{
	...
	{
		if (sprite_system.GetSprite(j).active && (sprite_system.GetSprite(j).player->controlmethod == human)
			&& (std::cmp_not_equal(j, i)))
		{
			if (GS::GetGame().pointvisible(spritePartsPos.x,
					spritePartsPos.y,
					sprite_system.GetSprite(j).player->camera)
				or (sprite_system.GetSprite(j).isspectator()
	...
	for (auto& sprite : sprite_system.GetActiveSprites())
	{
		...
		if ((sprite.player->controlmethod == human) && (j != i))
		{
				...
				|| (weaponsmsg.secondaryweaponnum != oldweaponsmsg[j][i].secondaryweaponnum))
			{
				if (GS::GetGame().pointvisible(spritePartsPos.x,
						spritePartsPos.y,
						sprite.player->camera)
					or (sprite.isspectator() && (sprite.player->port == 0))) // visible to sprite
				{
					gGlobalStateNetworkServer.GetServerNetwork()->SendData(
						&weaponsmsg, sizeof(weaponsmsg), sprite.player->peer, false);
	...
	for (auto& sprite : sprite_system.GetActiveSprites())
	{
		...
		if ((sprite.player->controlmethod == human) && (j != i))
		{
			...
			{
				gGlobalStateNetworkServer.GetServerNetwork()->SendData(
					&helmetmsg, sizeof(helmetmsg), sprite.player->peer, false);
...
void GlobalStateNetworkServerSprite::serverspritedeltasmouse(std::uint8_t i)
{
	...
	for (auto& sprite : sprite_system.GetActiveSprites())
	{
		...
		{
			gGlobalStateNetworkServer.GetServerNetwork()->SendData(
				&mouseaimmsg, sizeof(mouseaimmsg), sprite.player->peer, false);
			oldmouseaimmsg[j][i] = mouseaimmsg;
		}
	}
}

void GlobalStateNetworkServerSprite::serverhandleclientspritesnapshot(
	tmsgheader* netmessage, std::int32_t size, NetworkServer& /*network*/, TServerPlayer* player)
{
	...
	clientmsg = reinterpret_cast<pmsg_clientspritesnapshot>(netmessage);
	i = player->spritenum;
	...
	if ((sprite.weapon.num != clientmsg.weaponnum) || (sprite.secondaryweapon.num != clientmsg.secondaryweaponnum))
	{
		...
		scrptdispatcher.onweaponchange(
			i, clientmsg.weaponnum, clientmsg.secondaryweaponnum, clientmsg.ammocount, clientmsg.secondaryammocount);
	}
...
void GlobalStateNetworkServerSprite::serverhandleclientspritesnapshot_mov(
	tmsgheader* netmessage, std::int32_t size, NetworkServer& /*network*/, TServerPlayer* player)
{
	auto& sprite_system = SpriteSystem::Get();
	std::int32_t i = 0;
	...
	tmsg_clientspritesnapshot_mov& clientmovmsg = *reinterpret_cast<pmsg_clientspritesnapshot_mov>(netmessage);
	i = player->spritenum;
...
void GlobalStateNetworkServerSprite::serverhandleclientspritesnapshot_dead(
	tmsgheader* netmessage, std::int32_t size, NetworkServer& /*network*/, TServerPlayer* player)
{
	auto& sprite_system = SpriteSystem::Get();
	...
	clientdeadmsg = reinterpret_cast<pmsg_clientspritesnapshot_dead>(netmessage);
	i = player->spritenum;
