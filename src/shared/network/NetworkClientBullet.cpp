...
void clienthandlebulletsnapshot::Handle(NetworkContext* netmessage)
{
	...
	if (!bulletsnap->forced)
	{
		...
		if (bulletsnap->weaponnum == eagle_num) // Desert Eagle pellets
		{
			...
			const auto k = createbullet(a, bx, bulletsnap->weaponnum, bulletsnap->owner, 255, i, false, true);
			auto& b = bullet[k];
		...
		else if (style == bullet_style_shotgun) // SPAS-12 pellets
		{
			...
			for (d = 0; d <= 4; d++) // Remaining 5 pellets
			{
				bx.x = bstraight.x + ((Random() * 2 - 1) * bulletspread);
				bx.y = bstraight.y + ((Random() * 2 - 1) * bulletspread);
				const auto k = createbullet(a, bx, bulletsnap->weaponnum, bulletsnap->owner, 255, hm, false, true);
