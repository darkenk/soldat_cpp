...
void NetworkServer::ProcessEvents(PSteamNetConnectionStatusChangedCallback_t pInfo)
{
	...
	switch (pInfo->m_info.m_eState)
	{
		...
		{
			// NOTE that this is not called for ordinary disconnects, where we
			// use enet"s disconnect_now, which does not generate additional
			// events. Cleanup of Player is still performed explicitly.
			...
			LogWarn(
				LOG_NET, "Connection lost {} {}", pInfo->m_info.m_eEndReason, pInfo->m_info.m_szConnectionDescription);

			// call destructor; this releases any additional resources managed
			// for the connection, such as anti-cheat handles etc.
			std::erase_if(mPlayers,
				[&Player](const auto& v)
				{
					return Player == v.get();
				});
...
void NetworkServer::HandleMessages(SteamNetworkingMessage_t* msg)
{
	...
	// all the following commands can only be issued after the player has joined
	// the game.
	if ((player->spritenum == 0) or (sprite_system.GetSprite(player->spritenum).player.get() != player))
	{
		return;
	}
	...
	switch (packet->id)
	{
		case msgid_clientspritesnapshot:
			gGlobalStateNetworkServerSprite.serverhandleclientspritesnapshot(packet, msg->m_cbSize, *this, player);
			break;

		case msgid_clientspritesnapshot_mov:
			gGlobalStateNetworkServerSprite.serverhandleclientspritesnapshot_mov(packet, msg->m_cbSize, *this, player);
			break;

		case msgid_clientspritesnapshot_dead:
			gGlobalStateNetworkServerSprite.serverhandleclientspritesnapshot_dead(packet, msg->m_cbSize, *this, player);
			break;
...
auto NetworkServer::SendData(const std::byte* data, std::int32_t size, HSoldatNetConnection Peer, bool reliable) -> bool
{
...
auto GlobalStateNetworkServer::InitNetworkServer(const std::string_view& host, uint32_t port) -> bool
{
	mUdp = std::make_unique<NetworkServer>(host, port);
	return true;
}

auto GlobalStateNetworkServer::GetServerNetwork() -> NetworkServer*
{
	return mUdp.get();
}
