...
void setweaponactive(std::uint8_t id, std::uint8_t weaponnum, bool state)
{
	...
	if (id == 0)
	{
		for (auto& sprite : sprite_system.GetActiveSprites())
		{
			...
			{
				gGlobalStateNetworkServer.GetServerNetwork()->SendData(
					&wepmsg, sizeof(wepmsg), sprite.player->peer, true);
			}
		}
	}
...
void forceweapon(std::uint8_t id, std::uint8_t primary, std::uint8_t secondary, std::uint8_t ammo, std::uint8_t secammo)
{
	auto& sprite_system = SpriteSystem::Get();
...
void moveplayer(const std::uint8_t id, float x, float y)
{
	...
	for (auto& sprite : sprite_system.GetActiveSprites())
	{
		...
		{
			gGlobalStateNetworkServer.GetServerNetwork()->SendData(
				&movemsg, sizeof(movemsg), sprite.player->peer, true);
		}
	}
}
...
void modifyplayervelocity(const std::uint8_t id, float velx, float vely)
{
	...
	for (auto& sprite : sprite_system.GetActiveSprites())
	{
		if (sprite.player->controlmethod == human)
		{
			gGlobalStateNetworkServer.GetServerNetwork()->SendData(&velmsg, sizeof(velmsg), sprite.player->peer, true);
		}
	}
}

void forwardclient(
	std::uint8_t id, const std::string& /*targetip*/, std::int32_t targetport, const std::string& showmsg)
{
	auto& sprite_system = SpriteSystem::Get();
	...
	if ((sprite_system.GetSprite(id).active) && (sprite_system.GetSprite(id).player->controlmethod == human))
	{
		gGlobalStateNetworkServer.GetServerNetwork()->SendData(
...
void serverhandleclientfreecam(
