	...
	tmsgheader* netmessage, std::int32_t size, NetworkServer& /*network*/, TServerPlayer* player)
{
	...
	auto it = std::ranges::find_if(players,
		[&player](const auto& v)
		{
			return v.get() == player;
		});
	...
	#ifdef SCRIPT
	scrptdispatcher.onjointeam(SpriteSystem::Get().GetSprite(player.spritenum).num, player.team, player.team, true);
	#endif

	if (sprite_system.GetSprite(player->spritenum).active) // FIXME like above, it's always active is it not?
	{
	...
	if (j > 0)
	{
		...
		{
			gGlobalStateServer.kickplayer(player->spritenum, true, kick_flooding, twenty_minutes, "Join game flood");
		}
	}
	if (j == 0)
	{
...
#ifdef SERVER
	gGlobalStateNetworkServer.GetServerNetwork()->SendData(&playerslist, sizeof(playerslist), peer, true);
#else
	...
	else
	{
		if (banhw)
		{
			...
			else
			{
				...
				{
					bantimestr =
						inttostr((gGlobalStateBanSystem.bannedhwlist[banindex].time + 1) / 3600 / minsperhour) + 'h';
				}
				else
				{
					bantimestr = inttostr((gGlobalStateBanSystem.bannedhwlist[banindex].time + 1) / 3600) + 'm';
				}
			}
			// add ban reason
		...
		else
		{
			...
			else
			{
				...
				{
					bantimestr =
						inttostr((gGlobalStateBanSystem.bannediplist[banindex].time + 1) / 3600 / minsperhour) + 'h';
				}
				else
				{
					bantimestr = inttostr((gGlobalStateBanSystem.bannediplist[banindex].time + 1) / 3600) + 'm';
				}
			}
			// add ban reason
...
void serversendnewplayerinfo(std::uint8_t num, std::uint8_t jointype)
{
	...
	if (!sprite_system.GetSprite(num).player->demoplayer)
	{
		...
		for (auto& player : players)
		{
			...
			gGlobalStateNetworkServer.GetServerNetwork()->SendData(&newplayer, sizeof(newplayer), player->peer, true);
		}
	}
	else
	{
		...
		{
			newplayer.adoptspriteid = static_cast<std::uint8_t>(sprite_system.GetSprite(num).player->demoplayer);
...
void serverdisconnect()
{
	...
	{
		gGlobalStateNetworkServer.GetServerNetwork()->SendData(&servermsg, sizeof(servermsg), dstplayer->peer, true);
	}
...
void serverplayerdisconnect(std::uint8_t num, std::uint8_t why)
{
	...
	for (const auto& dstplayer : players)
	{
		gGlobalStateNetworkServer.GetServerNetwork()->SendData(&playermsg, sizeof(playermsg), dstplayer->peer, true);
	}
