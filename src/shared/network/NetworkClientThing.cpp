...
void clienthandlethingtaken::Handle(NetworkContext* netmessage)
{
	...
	if ((!thing.active)
		&& ((thingtakensnap->style == object_stationary_gun) || (thingtakensnap->style < object_ussocom)))
	{
		// request the new thing if not active
		requestthingmsg.header.id = msgid_requestthing;
		requestthingmsg.thingid = i;
		gGlobalStateNetworkClient.GetNetwork()->SendData(&requestthingmsg, sizeof(requestthingmsg), false);
		return;
	}
	...
	switch (thing.style)
	{
			...
			switch (CVar::sv_gamemode)
			{
				...
				case gamestyle_ctf:
					...
					else
					{
						switch (sprite_system.GetSprite(j).player->team)
						{
							case team_alpha:
							{
								bigcaptext = iif(sprite_system.IsPlayerSprite(j),
									_("You got the Blue Flag!"),
				...
				case gamestyle_inf:
					...
					else
					{
						...
						{
							bigcaptext = iif(
								sprite_system.IsPlayerSprite(j), _("You got the Objective!"), _("Objective captured!"));
		...
		case object_minigun:
		{
			...
			if ((sprite_system.IsPlayerSprite(thingtakensnap->who)) && !sprite_system.GetPlayerSprite().deadmeat)
			{
				clientspritesnapshot();
			}
		}
		...
		case object_rambo_bow:
		{
			...
			{
				gGlobalStateClientGame.bigmessage(_("You got the Bow!"), capturemessagewait, capture_message_color);
			}
			else
			{
		...
		case object_flamer_kit:
		{
			...
			if (sprite_system.IsPlayerSprite(thingtakensnap->who))
			{
				gGlobalStateClientGame.bigmessage(_("Flame God Mode!"), capturemessagewait, bonus_message_color);
		...
		case object_predator_kit:
		{
			...
			if (sprite_system.IsPlayerSprite(thingtakensnap->who))
			{
				gGlobalStateClientGame.bigmessage(_("Predator Mode!"), capturemessagewait, bonus_message_color);
			}
			thing.kill();
		}
		break;
		case object_vest_kit:
		{
			...
			if (sprite_system.IsPlayerSprite(thingtakensnap->who))
			{
				gGlobalStateClientGame.bigmessage(_("Bulletproof Vest!"), capturemessagewait, capture_message_color);
			}
			thing.kill();
		}
		break;
		case object_berserk_kit:
		{
			...
			if (sprite_system.IsPlayerSprite(thingtakensnap->who))
			{
				gGlobalStateClientGame.bigmessage(_("Berserker Mode!"), capturemessagewait, bonus_message_color);
			}
			thing.kill();
		}
		break;
		case object_cluster_kit:
		{
			...
			if (sprite_system.IsPlayerSprite(thingtakensnap->who))
			{
				gGlobalStateClientGame.bigmessage(_("Cluster Grenades!"), capturemessagewait, capture_message_color);
			}
			thing.kill();
		}
		break;
