...
static void sPreprocessSprites(
	SpriteSystem& spriteSystem, tmsg_playerslist* playerslistmsg, tvector2& pos, tvector2& vel)
{
...
void clientsendplayerinfo()
{
		...
		|| (CVar::sv_gamemode == gamestyle_htf))
	{
		if ((gGlobalStateClient.selteam < team_alpha)
			|| ((gGlobalStateClient.selteam > team_bravo) && (gGlobalStateClient.selteam < team_spectator)))
		{
...
void clientdisconnect(INetwork& client)
{
	...
	{ // send disconnection info to server
		...
		GS::GetConsoleLogFile().Log(
			string("Client Disconnect from ") + gGlobalStateNetworkClient.GetNetwork()->GetStringAddress(true));
		client.ProcessLoop();
...
void clienthandleplayerslist::Handle(NetworkContext* netmessage)
{
	...
	if (!gGlobalStateDemo.demoplayer.active())
	{
		GS::GetMainConsole().console(
			_("Connection accepted to") + ' ' + (gGlobalStateNetworkClient.GetNetwork()->GetStringAddress(true)),
			client_message_color);
	}

	gGlobalStateClient.gClientServerIP = gGlobalStateNetworkClient.GetNetwork()->GetStringAddress(false);
	...
	if (CVar::cl_servermods)
	{
		if (!modname.empty())
		{
			if (const tsha1digest checksum = sha1file(GS::GetGame().GetUserDirectory() + "mods/" + modname + ".smod");
				playerslistmsg->modchecksum == checksum)
			{
				if (PHYSFS_mount(pchar(GS::GetGame().GetUserDirectory() + "mods/" + modname + ".smod"),
						pchar(string("mods/") + modname + '/'),
						0)
					== 0)
				{
	...
	if (CVar::cl_runs < 3)
	{
		...
		{
			GS::GetMainConsole().console(
				_("(!) Jet fuel is map specific. There can be more or less on certain maps."), info_message_color);
		}
		else
		{
			GS::GetMainConsole().console(_("(!) To leave your current weapon after respawn"), info_message_color);
			GS::GetMainConsole().console(
				string("    ") + _("click anywhere outside the weapons menu."), info_message_color);
		}
...
void clienthandleunaccepted::Handle(NetworkContext* netmessage)
{
	...
	switch (unacceptedmsg->state)
	{
		...
		case banned_ip:
			gGlobalStateGameRendering.rendergameinfo(_("You have been banned on this server. Reason:") + ' ' + text);
			break;
...
void clienthandleservervars::Handle(NetworkContext* netmessage)
{
	...
	createdefaultweapons(
		CVar::sv_realisticmode, GS::GetWeaponSystem().GetGuns(), GS::GetWeaponSystem().GetDefaultGuns());
	...
	if (sprite_system.IsPlayerSpriteValid())
	{
		...
		sprite_system.GetPlayerSprite().applyweaponbynum(sprite_system.GetPlayerSprite().secondaryweapon.num, 2);
...
void clienthandlesynccvars::Handle(NetworkContext* netmessage)
{
	...
	std::int32_t const size = netmessage->size - (sizeof(varsmsg->header) + sizeof(varsmsg->itemcount));
...
namespace
{
	...
	protected:
		auto SendDataImpl(
			const std::byte* data, const std::int32_t size, const bool reliable, const source_location& /*unused*/)
			-> bool final
		{
	...
	auto verifyPlayer(const tsprite& sprite,
		const std::string& name,
		std::uint32_t shirtcolor,
		std::uint32_t pantscolor,
		std::uint32_t skincolor,
		std::uint32_t haircolor,
		std::uint32_t jetcolor,
		std::int32_t team,
		std::int32_t hairstyle,
		std::int32_t headcap,
		std::int32_t chain) -> bool
	{
		if (!sprite.player)
		{
			return false;
	...
	TEST_SUITE("NetworkClientConnection")
	{
		...
		TEST_CASE_FIXTURE(NetworkClientConnectionFixture, "sPreprocessSprites player appearance")
		{
			...
			const TestCase testCases[] = {
				...
				{	  .look = B5, .expectedHair = 0, .expectedHead = GFX::GOSTEK_HELM, .expectedChain = 0 }, // Helmet
				...
				{ .look = B2 | B7, .expectedHair = 2,				  .expectedHead = 0, .expectedChain = 1 }, // Combined hair + chain
			};
		...
		TEST_CASE_FIXTURE(NetworkClientConnectionFixture, "sPreprocessSprites handles 32 players")
		{
			...
			for (int i = 1; i <= max_sprites; i++)
			{
				...
				CHECK(verifyPlayer(sprite,
					expectedName,
					(0x100000 + idx) | 0xff000000,
					(0x200000 + idx) | 0xff000000,
					(0x300000 + idx) | 0xff000000,
					(0x400000 + idx) | 0xff000000,
					0x500000 + idx,
					(idx % 4) + 1,
