...
void kickforfaeviolation(tplayer player, std::string reason)
{
	...
	if (name == "")
		name = string("Unnamed player ") + const std::wstring
		   and (player.ip) + ':' + const std::wstring & (inttostr(player.port));
	...
	else
	{
		...
		else
		{
			...
			else
			{
				...
				if (!player.encactive)
				{
					debug(string("[AC] Network encryption enabled for ") + player.ip + ':' + inttostr(player.port));
