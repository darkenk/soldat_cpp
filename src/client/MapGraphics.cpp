...
auto GlobalStateMapGraphics::loadmaptexture(const string& texname, tgfxcolor colorkey) -> tgfximage*
{
...
void GlobalStateMapGraphics::updatescale(const tmapvertex p,
	tmapvertex q,
	...
	tvector2& scale)
{
	...
	d1 = sqrt((dx * dx) + (dy * dy));
	d2 = sqrt((du * du) + (dv * dv));
	sx = std::fabs((d1 * resolutionx * du) / d2) / (du * image->width());
	sy = std::fabs((d1 * resolutiony * dv) / d2) / (dv * image->height());
...
auto GlobalStateMapGraphics::gettexturetargetscale(tmapfile& mapfile, tgfximage* image) -> float
{
	...
	std::uint8_t alpha = 0;
	float area = NAN;
	float resolutionx = NAN;
	float resolutiony = NAN;
	tmapvertex a;
	tmapvertex b;
	tmapvertex c;
	...
	resolutionx = static_cast<float>(gGlobalStateClientGame.renderwidth) / gGlobalStateGame.gamewidth;
	resolutiony = static_cast<float>(gGlobalStateClientGame.renderheight) / gGlobalStateGame.gameheight;
...
void GlobalStateMapGraphics::loadmapgraphics(
	tmapfile& mapfile, bool bgforce, tmapcolor bgcolortop, tmapcolor bgcolorbtm)
{
	std::int32_t i = 0;
	std::int32_t j = 0;
	std::int32_t k = 0;
	...
	std::int32_t d = 0;
	std::int32_t level = 0;
	std::int32_t imgwidth = 0;
	std::int32_t imgheight = 0;
	std::int32_t proptotal = 0;
	std::int32_t animtotal = 0;
	std::int32_t animindex = 0;
	std::int32_t vbanimindex = 0;
	std::int32_t vbpropindex = 0;
	std::int32_t vbindex = 0;
	std::int32_t ibindex = 0;
	tgfxspritesheet* sheet = nullptr;
	pgfxsprite sprite = nullptr;
	tgfximage* image = nullptr;
	tgfximage* edgeimage = nullptr;
	std::vector<tgfxsprite*> edgesprites;
	tgfxcolor color{};
	pmapprop prop = nullptr;
	pmappolygon poly = nullptr;
	...
	//  Str: string;
	bool previsanim = false;
	std::int32_t scenerycount = 0;
	...
	{
		image = loadmaptexture(mapfile.textures[i], rgba(0, 0, 0, static_cast<std::uint8_t>(0)));
		...
		if (CVar::r_optimizetextures)
		{
			...
			imgwidth = max(2, min(image->width(), static_cast<std::int32_t>(std::round(scale * image->width()))));
			imgheight = max(2, min(image->height(), static_cast<std::int32_t>(std::round(scale * image->height()))));
		}
		...
		if (CVar::r_smoothedges)
		{
			...
			mg.edgetextures[i] = gfxcreatetexture(
				image->width(), image->height(), 4, image->getimagedata(0), mapfile.mapname + "smooth_edghes");
			...
			edgesprites[i]->texcoords.right = static_cast<float>(edgeimage->width()) / mg.edgetextures[i]->width();
			edgesprites[i]->texcoords.bottom = static_cast<float>(edgeimage->height()) / mg.edgetextures[i]->height();
		...
		for (i = low(mapfile.props); i <= high(mapfile.props); i++)
		{
			if (ispropactive(mapfile, i))
			{
				...
				scenerymaxsize[prop->style - 1].x = max(scenerymaxsize[prop->style - 1].x,
					MyFloat(MyFloat(std::fabs(prop->scalex * prop->width))
							* (MyFloat(gGlobalStateClientGame.renderheight) / gGlobalStateGame.gameheight)));
				scenerymaxsize[prop->style - 1].y = max(scenerymaxsize[prop->style - 1].y,
					MyFloat(MyFloat(std::fabs(prop->scaley * prop->height))
							* (MyFloat(gGlobalStateClientGame.renderheight) / gGlobalStateGame.gameheight)));
			}
		}
	...
	if (scenerycount > 0)
	{
		...
		for (i = low(mapfile.scenery); i <= high(mapfile.scenery); i++)
		{
			if (scenerycounters[i] > 0)
			{
				...
				auto str =
					gGlobalStateGameRendering.pngoverride("current_map/scenery-gfx" + mapfile.scenery[i].filename);
	...
	if (CVar::r_smoothedges)
	{
		...
		for (i = low(mapfile.polygons); i <= high(mapfile.polygons); i++)
		{
			...
			for (k = 0; k < 3; k++)
			{
				...
				if ((!map.collisiontestexcept(a, b, i + 1)) and (min(edge.a->color[3], edge.b->color[3]) > 128))
				{
	...
	{
		std::memcpy(
			mg.animationsbuffer.data(), vb.data(), static_cast<unsigned long>(4 * animtotal) * sizeof(tgfxvertex));
	}
	...
	for (i = low(mg.animations); i <= high(mg.animations); i++)
	{
		...
		mg.animationscmd[i] =
			&mg.props[(std::uint64_t)(mg.animationscmd[i]) >> 28][(std::uint64_t)(mg.animationscmd[i]) & 0xfffffff];
		...
		for (level = 0; level <= 1; level++)
		{
			for (i = 0; i <= edgecount - 1; i++)
			{
				if (edges[i]->level == level)
				{
					...
					if ((length(mg.edges[level]) == 0) || (cmd->texture != mg.edgetextures[edges[i]->textureindex]))
					{
					...
					gfxspritevertices(
						sprite, a.x, a.y, sprite->width, sprite->height, sx, 1, 0, 0.5, r, color, &vb[vbindex]);
	...
	d = max_sector * max(static_cast<float>(mapfile.sectorsdivision), static_cast<float>(ceil(0.5 * h / max_sector)));
	...
	{
		w = 260 * (static_cast<float>(gGlobalStateClientGame.renderwidth) / gGlobalStateGame.gamewidth);
	}
	else
	{
	...
	i = std::round(sx * (bounds.right - bounds.left)); // width
	j = std::round(sx * (bounds.bottom - bounds.top)); // height
	...
	if (gfxframebuffersupported())
	{
		...
		sprite->texcoords.right = static_cast<float>(i) / sprite->texture->width();
		sprite->texcoords.bottom = static_cast<float>(j) / sprite->texture->height();

		if (CVar::r_scaleinterface)
		{
			sprite->scale = static_cast<float>(gGlobalStateGame.gamewidth) / gGlobalStateClientGame.renderwidth;
		...
		u = static_cast<float>((n * i)) / texture->width();
		v = static_cast<float>((n * j)) / texture->height();
		color = rgba(0xffffff);
...
void GlobalStateMapGraphics::updateprops(double t)
{
	...
	for (i = low(mg->animations); i <= high(mg->animations); i++)
	{
		...
		{
			accum = 0;
			duration = static_cast<float>(mg->animduration[i]) / 100;
			animtime = trunc(100 * (t - duration * std::trunc(static_cast<float>(t) / duration)));
			sprite = mg->animations[i];
...
void GlobalStateMapGraphics::worldtominimap(float x, float y, MyFloat& ox, MyFloat& oy) const
{
