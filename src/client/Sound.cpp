...
static constexpr void Add(
	std::array<std::string_view, ToUint32(SfxEffect::COUNT)>& ref, SfxEffect sample, std::string_view name)
{
...
static constexpr auto GenerateSampleFileNames() -> std::array<std::string_view, ToUint32(SfxEffect::COUNT)>
{
...
public:
	...
	void fplaysound(
		SfxEffect samplenum, float listenerx, float listenery, float emitterx, float emittery, std::int32_t chan)
	{
		...
		if (chan != -1)
		{
			...
			LogDebug(
				AUDIO, "Play sound {}; sample num {}; channel {}", sample_files[ToUint32(samplenum)], samplenum, chan);
		}
	}
...
auto GlobalStateSound::loadsample(const std::string_view& name, const tsoundsample& samp) -> tsoundsample
{
...
void GlobalStateSound::fplaysound(
	SfxEffect samplenum, float listenerx, float listenery, float emitterx, float emittery, std::int32_t chan)
{
...
{
	fplaysound(sample, gGlobalStateClient.camerax, gGlobalStateClient.cameray, emitter.x, emitter.y, defaultchannel);
}

void GlobalStateSound::playsound(SfxEffect sample, const tvector2& emitter, int32_t channel)
{
	fplaysound(sample, gGlobalStateClient.camerax, gGlobalStateClient.cameray, emitter.x, emitter.y, channel);
}

auto GlobalStateSound::stopsound(std::int32_t channel) -> bool
{
