...
static unsigned char nuklear_ui_frag_spv[] = { 0x03,
	0x02,
	0x23,
	0x07,
	0x00,
...
static unsigned char nuklear_ui_vert_spv[] = { 0x03,
	0x02,
	0x23,
	0x07,
	0x00,
...
NK_INTERN void nk_create_or_resize_buffer(SDL_GPUBuffer** buffer,
	uint32_t* old_size,
	uint32_t new_size,
...
void nk_sdl_prepare_render_data(
	enum nk_anti_aliasing AA, SDL_GPUCommandBuffer* command_buffer, unsigned int fb_width, unsigned int fb_height)
{
	...
	static const struct nk_draw_vertex_layout_element vertex_layout[] = {
		...
		{ .attribute = NK_VERTEX_TEXCOORD, .format = NK_FORMAT_FLOAT, .offset = NK_OFFSETOF(struct nk_sdl_vertex, uv) },
	...
	{
		nk_create_or_resize_buffer(&sdl.vertex_buffer,
			&sdl.vertex_buffer_size,
			vertex_size,
			SDL_GPU_BUFFERUSAGE_VERTEX,
			"nuklear_vertex_buffer");
	}
	if (sdl.index_buffer == nullptr || sdl.index_buffer_size < index_size)
	{
		nk_create_or_resize_buffer(
			&sdl.index_buffer, &sdl.index_buffer_size, index_size, SDL_GPU_BUFFERUSAGE_INDEX, "nuklear_index_buffer");
	}
	...
	SDL_GPUTransferBuffer* vertex_transferbuffer = SDL_CreateGPUTransferBuffer(sdl.device, &vertex_transferbuffer_info);
	assert(vertex_transferbuffer != nullptr
		   && "Failed to create the vertex transfer buffer, call SDL_GetError() for more information");
	SDL_GPUTransferBuffer* index_transferbuffer = SDL_CreateGPUTransferBuffer(sdl.device, &index_transferbuffer_info);
	assert(index_transferbuffer != nullptr
		   && "Failed to create the index transfer buffer, call SDL_GetError() for more information");

	auto* vtx_dst = static_cast<nk_sdl_vertex*>(SDL_MapGPUTransferBuffer(sdl.device, vertex_transferbuffer, true));
	auto* idx_dst = static_cast<nk_draw_index*>(SDL_MapGPUTransferBuffer(sdl.device, index_transferbuffer, true));
...
NK_INTERN void nk_create_shaders()
{
	...
	SDL_SetStringProperty(vertex_shader_info.props, SDL_PROP_GPU_SHADER_CREATE_NAME_STRING, "nuklear_vertex_shader");
	...
	SDL_SetStringProperty(
		fragment_shader_info.props, SDL_PROP_GPU_SHADER_CREATE_NAME_STRING, "nuklear_fragment_shader");
	...
	assert(sdl.vertex_shader != nullptr && "Failed to create vertex shader, call SDL_GetError() for more information");
	assert(
		sdl.fragment_shader != nullptr && "Failed to create fragment shader, call SDL_GetError() for more information");
...
NK_INTERN void nk_sdl_create_font_sampler()
{
	...
	SDL_SetStringProperty(sampler_info.props, SDL_PROP_GPU_SAMPLER_CREATE_NAME_STRING, "nuklear_font_sampler");

	sdl.font_sampler = SDL_CreateGPUSampler(sdl.device, &sampler_info);
	sdl.font_binding.sampler = sdl.font_sampler;
	assert(sdl.font_sampler != nullptr && "Failed to create font sampler, call SDL_GetError() for more information");
...
NK_INTERN void nk_sdl_create_graphics_pipeline()
{
	...
	color_target_desc[0].format = SDL_GetGPUSwapchainTextureFormat(sdl.device,
	...
	SDL_SetStringProperty(
		pipeline_info.props, SDL_PROP_GPU_GRAPHICSPIPELINE_CREATE_NAME_STRING, "nuklear_graphics_pipeline");

	sdl.pipeline = SDL_CreateGPUGraphicsPipeline(sdl.device, &pipeline_info);
	assert(sdl.pipeline != nullptr && "Failed to create graphics pipeline, call SDL_GetError() for more information");
	...
	uint32_t fb_height)
{
	...
	if (sdl.vertex_buffer_size > 0)
	{
		...
		SDL_BindGPUIndexBuffer(render_pass,
			...
			sizeof(nk_draw_index) == 2 ? SDL_GPU_INDEXELEMENTSIZE_16BIT : SDL_GPU_INDEXELEMENTSIZE_32BIT);
	}
...
NK_INTERN void nk_sdl_device_upload_atlas(const void* image, int width, int height)
{
	...
	// Create the Image:
	{
		...
		SDL_SetStringProperty(texture_info.props, SDL_PROP_GPU_TEXTURE_CREATE_NAME_STRING, "nuklear_font_texture");
	...
	// Create all the upload structures and upload:
	{
		...
		SDL_GPUTransferBuffer* transferbuffer = SDL_CreateGPUTransferBuffer(sdl.device, &transferbuffer_info);
...
NK_API void nk_sdl_render(
	enum nk_anti_aliasing /*AA*/, SDL_GPUCommandBuffer* command_buffer, SDL_GPURenderPass* render_pass)
{
	/* setup global state */
	struct nk_sdl_device* dev = &sdl.ogl;

	nk_sdl_setup_render_state(sdl.pipeline, command_buffer, render_pass, sdl.display_width, sdl.display_height);

	{
		...
		SDL_Rect const scissor_rect{ 0, 0, static_cast<int>(sdl.display_width), static_cast<int>(sdl.display_height) };
