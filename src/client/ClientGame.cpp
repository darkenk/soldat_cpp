...
void GlobalStateClientGame::bigmessage(const std::string& text, std::int32_t delay, std::uint32_t col) const
{
	float w = NAN;
	float s = NAN;
	...
	{
		gGlobalStateInterfaceGraphics.bigposx[1] =
			gGlobalStateInterfaceGraphics.bigposx[1] * (static_cast<float>(gGlobalStateGame.gamewidth) / renderwidth);
	}
}
...
void GlobalStateClientGame::tabcomplete()
{
	...
	if (chattextlen > offset) // Dont complete if chat is empty
	{
		for (i = continuedtabcompleteplayer; i <= (continuedtabcompleteplayer + max_players - 1); i++)
		{
			...
			if (sprite.IsActive() && (!sprite.player->demoplayer) && (!sprite_system.IsPlayerSprite(next)))
			{
				if ((completionbase.empty()) || std::string::npos != sprite.player->name.find(completionbase))
				{
...
auto GlobalStateClientGame::getgamefps() const -> std::int32_t
{
...
void GlobalStateClientGame::gameloop()
{
	...
	{ // frame rate independant code
		...
		// Radio Cooldown
		if ((GS::GetGame().GetMainTickCounter() % second == 0) && (gGlobalStateClient.radiocooldown > 0)
			&& (CVar::sv_radio))
		{
		...
		if ((sprite_system.IsPlayerSpriteValid()) && (!gGlobalStateDemo.demoplayer.active()))
		{
			...
			gGlobalStateNetworkClient.noheartbeattime = std::max(gGlobalStateNetworkClient.noheartbeattime, 0);
			...
			if (gGlobalStateClient.connection == INTERNET)
			{
				if (sprite.IsActive())
				{
					if (!sprite.deadmeat)
					{
						if ((GS::GetGame().GetMainTickCounter() % static_cast<std::int32_t>(std::round(7 * adjust))
								== 1)
							&& (GS::GetGame().GetMainTickCounter() % static_cast<std::int32_t>(std::round(5 * adjust))
								!= 0))
						{
							clientspritesnapshot();
						}
						if ((GS::GetGame().GetMainTickCounter() % static_cast<std::int32_t>(std::round(5 * adjust))
								== 0)
					...
					else if (GS::GetGame().GetMainTickCounter() % static_cast<std::int32_t>(std::round(30 * adjust))
							 == 0)
					{
			...
			else if (gGlobalStateClient.connection == LAN)
			{
				if (!sprite.deadmeat)
				{
					...
					if ((GS::GetGame().GetMainTickCounter() % static_cast<std::int32_t>(std::round(3 * adjust)) == 0)
						|| forceclientspritesnapshotmov)
					{
						clientspritesnapshotmov();
					}
				}
				else if (GS::GetGame().GetMainTickCounter() % static_cast<std::int32_t>(std::round(15 * adjust)) == 0)
				{
					clientspritesnapshotdead();
				}
			}
	...
	if (shouldrenderframes && ((currenttime - frametiming.prevrendertime) >= frametiming.mindeltatime))
	{
		...
		{
			gGlobalStateGameRendering.renderframe(frametiming.elapsed - (dt * (1 - framepercent)), framepercent, false);
		}
	}
