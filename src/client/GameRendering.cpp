...
void GlobalStateGameRendering::loadmodinfo()
{
	...
	auto interfaceinistream = ReadAsFileStream(
		fs, (string("custom-interfaces/") + gGlobalStateGameRendering.gamerenderingparams.interfacename + "/mod.ini"));
...
auto GlobalStateGameRendering::getimagescale(const std::string& imagepath) -> float
{
	...
	std::string const intdir =
		string("/custom-interfaces/") + lowercase(gGlobalStateGameRendering.gamerenderingparams.interfacename) + '/';
...
void GlobalStateGameRendering::loadmaintextures()
{
	...
	const auto count = std::count_if(GFXData.begin(),
		GFXData.end(),
		[](const auto& d)
		{
			return d.Group != GFXG::INTERFACE;
		});
...
{
	if (gGlobalStateInterfaceGraphics.loadinterfacedata(gGlobalStateGameRendering.gamerenderingparams.interfacename))
	{
...
void GlobalStateGameRendering::loadfonts()
{
	...
	const float s = iif(CVar::r_scaleinterface,
	...
	for (std::int32_t i = 0; i < fontstyles.size(); i++)
	{
		...
		fontstyles[i].tableindex =
			gfxsetfont(fontstyles[i].font, fontstyles[i].size, fontstyles[i].flags, fontstyles[i].stretch);
	}
}
...
auto GlobalStateGameRendering::initgamegraphics() -> bool
{
	...
	{
		actionsnaptexture =
			gfxcreaterendertarget(gGlobalStateClientGame.renderwidth, gGlobalStateClientGame.renderheight, 4, true);
	}
	...
	if (gfxframebuffersupported())
	{
			...
			|| (gGlobalStateClientGame.windowheight != gGlobalStateClientGame.renderheight))
		{
			...
			{
				rendertargetaa = gfxcreaterendertarget(
					gGlobalStateClientGame.renderwidth, gGlobalStateClientGame.renderheight, 4, false);
...
void GlobalStateGameRendering::interpolatestate(float p, tinterpolationstate& s, bool paused)
{
	...
	static const std::set<std::int32_t> kit_styles = { object_medical_kit,
		object_grenade_kit,
		object_flamer_kit,
		object_predator_kit,
		object_vest_kit,
	...
	gGlobalStateClientGame.mx = lerp(gGlobalStateClientGame.mouseprev.x, gGlobalStateClientGame.mx, p);
	gGlobalStateClientGame.my = lerp(gGlobalStateClientGame.mouseprev.y, gGlobalStateClientGame.my, p);
		...
		for (i = 1; i <= max_sparks; i++)
		{
			if (gGlobalStateGame.spark[i].active)
			{
				...
				gGlobalStateSparks.GetSparkParts().pos[j] =
					lerp(gGlobalStateSparks.GetSparkParts().oldpos[j], gGlobalStateSparks.GetSparkParts().pos[j], p);
...
void GlobalStateGameRendering::restorestate(tinterpolationstate& s)
{
	...
	{
		i = sprite.num;
		std::memcpy(&sprite.skeleton.pos[1], &s.spritepos[i][1], sizeof(tvector2) * length(s.spritepos[i]));
	}
	...
	for (i = 1; i <= max_things; i++)
	{
		if (things[i].active)
		{
			std::memcpy(&things[i].skeleton.pos[1], &s.thingpos[i][1], sizeof(tvector2) * length(s.thingpos[i]));
		}
	}
}
...
void GlobalStateGameRendering::renderframe(double timeelapsed, double framepercent, bool paused)
{
	...
	if ((gGlobalStateClientGame.showscreen != 0U) && gGlobalStateClientGame.actionsnaptaken)
	{
		...
		gfxtextpixelratio(vector2(w / gGlobalStateClientGame.renderwidth, h / gGlobalStateClientGame.renderheight));
	...
	if (rendertarget != nullptr)
	{
		...
		{
			s = static_cast<float>(gGlobalStateClientGame.windowwidth) / gGlobalStateClientGame.screenwidth;
			w = w * s;
		...
		{
			s = static_cast<float>(gGlobalStateClientGame.windowheight) / gGlobalStateClientGame.screenheight;
			h = h * s;
		...
		gfxviewport(0, 0, gGlobalStateClientGame.windowwidth, gGlobalStateClientGame.windowheight);
		gfxclear(rgba(0));
		gfxtransform(gfxmat3ortho(0, gGlobalStateClientGame.windowwidth, 0, gGlobalStateClientGame.windowheight));
		...
		gfxbegin();
		gfxdrawquad(rt,
			gfxvertex(dx + 0, dy + 0, 0, v, rgba(0xffffff)),
...
auto GlobalStateGameRendering::getsizeconstraint(std::int32_t id, std::int32_t& w, std::int32_t& h) -> bool
{
	const std::array<std::int32_t, 35> weapons_list = {
		{ GFX::WEAPONS_AK74,
			GFX::WEAPONS_AK74_2,
			GFX::WEAPONS_AK74_FIRE,
			GFX::WEAPONS_MINIMI,
			GFX::WEAPONS_MINIMI_2,
			GFX::WEAPONS_RUGER,
			GFX::WEAPONS_RUGER_2,
			GFX::WEAPONS_MP5,
			GFX::WEAPONS_MP5_2,
			...
			GFX::WEAPONS_M79_2,
			GFX::WEAPONS_DEAGLES,
			GFX::WEAPONS_DEAGLES_2,
			GFX::WEAPONS_N_DEAGLES,
			GFX::WEAPONS_N_DEAGLES_2,
			GFX::WEAPONS_STEYR,
			GFX::WEAPONS_STEYR_2,
			GFX::WEAPONS_BARRETT,
			GFX::WEAPONS_BARRETT_2,
			GFX::WEAPONS_MINIGUN,
			GFX::WEAPONS_MINIGUN_2,
			GFX::WEAPONS_SOCOM,
			GFX::WEAPONS_SOCOM_2,
			GFX::WEAPONS_N_SOCOM,
			GFX::WEAPONS_N_SOCOM_2,
			GFX::WEAPONS_BOW,
			GFX::WEAPONS_BOW_S,
			GFX::WEAPONS_FLAMER,
			GFX::WEAPONS_FLAMER_2,
			GFX::WEAPONS_KNIFE,
			GFX::WEAPONS_KNIFE2,
			GFX::WEAPONS_CHAINSAW,
			GFX::WEAPONS_CHAINSAW2 }
	};
...
auto GlobalStateGameRendering::dotextureloading(bool finishloading) -> bool
{
	...
	if (mainspritesheet->loading() != mainloading)
	{
		...
		for (i = 0; i <= mainspritesheet->texturecount() - 1; i++)
		{
			s = s
			  + fmt::format(
				  "{}x{} ", mainspritesheet->gettexture(i)->width(), mainspritesheet->gettexture(i)->height());
	...
	if (interfacespritesheet->loading() != interfaceloading)
	{
		...
		for (i = 0; i <= interfacespritesheet->texturecount() - 1; i++)
		{
			s = s
			  + fmt::format("{}x{} ",
				  interfacespritesheet->gettexture(i)->width(),
...
{
	gfxsetfont(
		fontstyles[style].font, scale * fontstyles[style].size, fontstyles[style].flags, fontstyles[style].stretch);
}
...
void GlobalStateGameRendering::gfxlogcallback(const std::string& s)
{
	LogDebugG("[GFX] {}", s);
}
...
public:
	...
	[[nodiscard]] std::string getFileExtensionWithDot() const override { return ".png"; }
	...
	void cleanUpReceived(std::string receivedPath) const override { remove(receivedPath.c_str()); };
...
namespace
{
	...
	TEST_SUITE("GameRenderingSuite")
	{
		...
		TEST_CASE_FIXTURE(GameRenderingFixture, "Render text" * doctest::skip(false))
		{
			...
			auto data = gfxsavescreen(0, 0, gGlobalStateClientGame.renderwidth, gGlobalStateClientGame.renderheight);
			PngWriter const writer(
				gGlobalStateClientGame.renderwidth, gGlobalStateClientGame.renderheight, std::move(data));
