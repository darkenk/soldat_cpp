...
FGlobalStateControlGame gGlobalStateControlGame{

};

void FGlobalStateControlGame::clearchattext()
{
...
auto FGlobalStateControlGame::filterchattext(const std::string& str1) -> std::string
{
	std::string result;
	result = std::accumulate(str1.begin(),
		str1.end(),
		result,
		[](const auto& out, const auto& c)
		{
			if ((c >= '\40') && (c != '\177'))
			{
				return out + c;
...
auto FGlobalStateControlGame::chatkeydown(std::uint8_t keymods, SDL_Keycode keycode) -> bool
{
	...
	if (length(gGlobalStateClientGame.chattext) > 0)
	{
		...
		else if (keymods == km_none)
		{
			...
			switch (keycode)
			{
				...
				{
					gGlobalStateClientGame.chatchanged = true;
					if ((gGlobalStateClientGame.cursorposition > 1) || (length(gGlobalStateClientGame.chattext) == 1))
					{
						gGlobalStateClientGame.currenttabcompleteplayer = 0;
						gGlobalStateClientGame.chattext.erase(
							gGlobalStateClientGame.cursorposition - 1, length(gGlobalStateClientGame.chattext));
						gGlobalStateClientGame.cursorposition -= 1;
						if (length(gGlobalStateClientGame.chattext) == 0)
						{
							gGlobalStateClientGame.chattext = gGlobalStateClientGame.lastchattext;
				...
				case SDLK_KP_ENTER:
				{
					...
					if (sprite_system.IsPlayerSpriteValid())
					{
						if (gGlobalStateControlGame.votekickreasontype)
						{
							...
							{
								clientvotekick(gGlobalStateGameMenus.kickmenuindex,
									false,
						...
						else
						{
							clientsendstringmessage(
								gGlobalStateClientGame.chattext.substr(1), gGlobalStateClientGame.chattype);
						}
					}
...
auto FGlobalStateControlGame::menukeydown(std::uint8_t keymods, SDL_Scancode keycode) -> bool
{
	bool result = false;
	result = false;
	...
	if ((keymods == km_none) && (keycode == SDL_SCANCODE_ESCAPE))
	{
		...
		{
			gGlobalStateGameMenus.gamemenushow(gGlobalStateGameMenus.escmenu, !gGlobalStateGameMenus.escmenu->active);
		}
	}
	else if ((keycode >= SDL_SCANCODE_1) && (keycode <= SDL_SCANCODE_0))
	{
		if (gGlobalStateGameMenus.teammenu->active)
		{
			if (keymods == km_none)
			{
				result = gGlobalStateGameMenus.gamemenuaction(
					gGlobalStateGameMenus.teammenu, ((keycode - SDL_SCANCODE_1) + 1) % 10);
			}
		}
		else if (gGlobalStateGameMenus.escmenu->active)
		{
			if (keymods == km_none)
			{
				result = gGlobalStateGameMenus.gamemenuaction(gGlobalStateGameMenus.escmenu, keycode - SDL_SCANCODE_1);
			}
		}
		else if (gGlobalStateGameMenus.limbomenu->active)
		{
			switch (keymods)
			{
				case km_none:
					result =
						gGlobalStateGameMenus.gamemenuaction(gGlobalStateGameMenus.limbomenu, keycode - SDL_SCANCODE_1);
					break;
				case km_ctrl:
					result = gGlobalStateGameMenus.gamemenuaction(
						gGlobalStateGameMenus.limbomenu, keycode - SDL_SCANCODE_1 + 10);
					break;
			}
		}
	}
...
auto FGlobalStateControlGame::keydown(SDL_KeyboardEvent& keyevent) -> bool
{
	auto& sprite_system = SpriteSystem::Get();
	std::int32_t i = 0;
	...
	keymods = (ord(0 != (keyevent.mod & SDL_KMOD_ALT)) << 0) | (ord(0 != (keyevent.mod & SDL_KMOD_CTRL)) << 1)
	...
	if (keymods == km_none)
	{
		switch (keycode)
		{
			case SDL_SCANCODE_PAGEDOWN:
			{
				...
				{
					gGlobalStateInterfaceGraphics.fragsscrolllev += ord(
						gGlobalStateInterfaceGraphics.fragsscrolllev < gGlobalStateInterfaceGraphics.fragsscrollmax);
				}
			}
			break;
			...
			case SDL_SCANCODE_F10:
			{
				...
				{
					result = true;
					if ((GS::GetGame().GetMapchangecounter() < 0) || (GS::GetGame().GetMapchangecounter() > 99999999))
					{
			...
			case SDL_SCANCODE_3:
			{
				...
				if ((gGlobalStateClientGame.chattext.empty()) && (CVar::sv_radio) && gGlobalStateClient.showradiomenu)
				{
					result = true;
	...
	if (action == taction::sniperline)
	{
		...
		{
			gGlobalStateClient.sniperline_client_hpp =
				static_cast<std::uint8_t>(static_cast<std::uint8_t>(gGlobalStateClient.sniperline_client_hpp) == 0U);
		}
	}
	...
	{
		if ((gGlobalStateClientGame.chattext.empty()) && (CVar::sv_radio) && (sprite_system.IsPlayerSpriteValid())
			and (sprite_system.GetPlayerSprite().isnotspectator)())
		{
	...
	else if (action == taction::chat)
	{
		if (gGlobalStateClientGame.chattext.empty())
		{
			...
			if ((CVar::sv_survivalmode) && sprite_system.GetPlayerSprite().isspectator() && !game.GetSurvivalEndRound()
	...
	else if (action == taction::snap)
	{
		...
		{
			gGlobalStateClientGame.showscreen =
				static_cast<std::uint8_t>(static_cast<std::uint8_t>(gGlobalStateClientGame.showscreen) == 0U);
			...
			&& !gGlobalStateGameMenus.escmenu->active && !sprite_system.GetPlayerSprite().isspectator())
		{
			...
			else
			{
				...
				if (!gGlobalStateGameMenus.limbomenu->active
					or (((prinum != noweapon_num) || (pricount == 0)) && ((secnum != noweapon_num) || (seccount == 0))))
				{
...
auto FGlobalStateControlGame::keyup(SDL_KeyboardEvent& keyevent) -> bool
{
	std::uint8_t keymods = 0;
	SDL_Scancode keycode;
	pbind bind = nullptr;
...
void FGlobalStateControlGame::RegisterEventHandlers(std::shared_ptr<FSdlApp>& InApp)
{
	InApp->RegisterEventHandler(SDL_EVENT_KEY_DOWN,
		[this](SDL_Event& InEvent)
		{
			if (!keydown(InEvent.key))
			{
				gGlobalStateInput.keystatus[InEvent.key.scancode] = true;
			}
			return true;
		});
	InApp->RegisterEventHandler(SDL_EVENT_KEY_UP,
		[this](SDL_Event& InEvent)
		{
			gGlobalStateInput.keystatus[InEvent.key.scancode] = false;
			keyup(InEvent.key);
			return true;
		});
	InApp->RegisterEventHandler(SDL_EVENT_MOUSE_BUTTON_DOWN,
		[](SDL_Event& InEvent)
		{
			if (!gGlobalStateGameMenus.gamemenuclick())
			{
				gGlobalStateInput.keystatus[InEvent.button.button + 300] = true;
			}
			return true;
		});

	InApp->RegisterEventHandler(SDL_EVENT_MOUSE_BUTTON_UP,
		[](SDL_Event& InEvent)
		{
			gGlobalStateInput.keystatus[InEvent.button.button + 300] = false;
			return true;
		});

	InApp->RegisterEventHandler(SDL_EVENT_TEXT_INPUT,
		[this](SDL_Event& InEvent)
		{
			std::string Str1;

			bool ChatEnabled = length(gGlobalStateClientGame.chattext) > 0;
			if (ChatEnabled)
			{
				Str1 = InEvent.text.text[0];
	...
	InApp->RegisterEventHandler(SDL_EVENT_MOUSE_MOTION,
		[](SDL_Event& InEvent)
		{
			if (0 != (SDL_GetWindowFlags(gGlobalStateInput.gamewindow) & SDL_WINDOW_INPUT_FOCUS))
			{
				gGlobalStateClientGame.mx = max(0.F,
					min(static_cast<float>(gGlobalStateGame.gamewidth),
						gGlobalStateClientGame.mx + (InEvent.motion.xrel * CVar::cl_sensitivity)));
				gGlobalStateClientGame.my = max(0.F,
					min(static_cast<float>(gGlobalStateGame.gameheight),
						gGlobalStateClientGame.my + (InEvent.motion.yrel * CVar::cl_sensitivity)));
