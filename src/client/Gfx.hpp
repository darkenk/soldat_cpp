...
struct tgfxrect
{
	...
	float width() { return std::abs(right - left); }
	float height() { return std::abs(top - bottom); }
};
...
struct trect
{
	...
	std::int32_t width() { return std::abs(right - left); }
	std::int32_t height() { return std::abs(top - bottom); }
};
...
public:
	...
	void update(
		std::int32_t x, std::int32_t y, std::int32_t w, std::int32_t h, std::uint8_t* data, std::int32_t frame = 0);
	void premultiply();
	void resize(std::int32_t w, std::int32_t h);
	std::int32_t width() { return fwidth; };
	std::int32_t height() { return fheight; };
	std::int32_t components() { return fcomponents; }
	std::int32_t numframes() { return fnumframes; }
};
...
public:
	...
	std::int32_t height() { return fheight; }
	std::int32_t components() { return fcomponents; }
	std::int32_t samples() { return fsamples; }

	std::int32_t handle() { return fhandle; }

	SDL_GPUTexture* getTexture() { return mTexture; }
};
...
public:
	...
	std::int32_t capacity() { return fcapacity; };

	std::int32_t handle() { return fhandle; }

	SDL_GPUBuffer* getBuffer() { return mBuffer; }
};
...
public:
	...
	std::uint32_t handle() { return fhandle; }
...
public:
	...
	void finishloading();
	std::int32_t spritecount() { return getspritecount(); };
	std::int32_t texturecount() { return gettexturecount(); };
...
void gfxsavescreen(
	const std::string& filename, std::int32_t x, std::int32_t y, std::int32_t w, std::int32_t h, bool async = true);
...
// texture
tgfxtexture* gfxcreatetexture(std::int32_t w,
	std::int32_t h,
	std::int32_t c /*= 4*/,
	std::uint8_t* data /*= 0*/,
	const std::string_view& debug_name);
tgfxtexture* gfxcreaterendertarget(std::int32_t w, std::int32_t h, std::int32_t c = 4, bool msaa = false);
...
// vertex buffer
tgfxvertexbuffer* gfxcreatebuffer(std::int32_t capacity, bool _static = false, pgfxvertex data = nullptr);
...
// index buffer
tgfxindexbuffer* gfxcreateindexbuffer(std::int32_t capacity, bool _static = false, std::uint16_t* data = nullptr);
