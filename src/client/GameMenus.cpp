...
void GlobalStateGameMenus::initbutton(pgamemenu menu,
	std::int32_t button,
...
auto GlobalStateGameMenus::gamemenuaction(pgamemenu menu, std::int32_t buttonindex) -> bool
{
	...
	if ((buttonindex >= 0) && (buttonindex < length(menu->button)) && menu->button[buttonindex].active)
	{
		if (menu == escmenu)
		{
			result = true;
			...
			else
			{
				switch (buttonindex)
				{
					...
					{ // prev
						...
						while (!(sprite_system.GetSprite(kickmenuindex).IsActive()
								 or sprite_system.GetSprite(kickmenuindex).player->demoplayer))
						{
							kickmenuindex = ((max_sprites + kickmenuindex - 2) % max_sprites) + 1;
						}
				...
				switch (buttonindex)
				{
					...
					{ // vote map
						...
						clientsendstringmessage(
							string("votemap ") + (gGlobalStateNetworkClient.votemapname), msgtype_cmd);
					}
					break;
				}
			}
		...
		else if ((menu == limbomenu) && (sprite_system.IsPlayerSpriteValid()))
		{
			...
			if (weaponSystem.IsEnabled(i) && (GS::GetGame().GetWeaponsel()[gGlobalStateClient.mysprite][i] == 1))
			{
				if (i <= 10)
				{
					...
					if (sprite_system.GetPlayerSprite().selweapon > 0)
					{
						...
						if (!sprite_system.GetPlayerSprite().deadmeat
							and sprite_system.GetPlayerSprite().weapon.num != bow_num
							and sprite_system.GetPlayerSprite().weapon.num != bow2_num)
						{
				...
				else
				{
					...
					sprite_system.GetPlayerSprite().applyweaponbynum(GS::GetWeaponSystem().GetGuns()[i].num, 2);
					...
					{
						gamemenushow(limbomenu, false);
						sprite_system.GetPlayerSprite().SetFirstWeapon(sprite_system.GetPlayerSprite().secondaryweapon);
						sprite_system.GetPlayerSprite().SetSecondWeapon(GS::GetWeaponSystem().GetGuns()[noweapon]);
					}
