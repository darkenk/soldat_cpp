...
void setupvertexattributes(tgfxvertexbuffer *buffer)
{
    if (gfxcontext.boundbuffer != buffer)
    {
        ...
        if (gfxcontext.shaderprogram != 0)
        {
            ...
            glVertexAttribPointer(2, 4, GL_UNSIGNED_BYTE, GL_TRUE, sizeof(tgfxvertex),
                                  (void *)(16));
        }
        else
        {
...
void OpenGLDebug(GLenum source, GLenum type, GLuint id, GLenum severity, GLsizei length,
                 const GLchar *message, const void *userParam)
{
    LogDebugG("[GL] {}", message);
}

void OpenGLGladDebug(const char *name, void * /*funcptr*/, int /*len_args*/, ...)
{
...
bool gfxinitcontext(SDL_Window *wnd, bool dithering, bool fixedpipeline)
{
    ...
    requiredfunctions =
        ...
        assigned(glViewport) && assigned(glClearColor) && assigned(glClear) &&
        assigned(glDrawArrays) && assigned(glDrawElements) && assigned(glReadPixels) &&
        assigned(glTexEnvf) && assigned(glGetIntegerv);
             ...
             std::int32_t count)
{
    ...
    glDrawElements(GL_TRIANGLES, count, GL_UNSIGNED_SHORT,
                   (void *)(sizeof(std::uint16_t) * offset));
}
...
tgfxtexture *gfxcreaterendertarget(std::int32_t w, std::int32_t h, std::int32_t c, bool msaa)
{
    ...
    if (gfxframebuffersupported())
        gfxcreaterendertarget_result =
            new tgfxtexture(w, h, c, true, msaa, nullptr, "rendertarget");
...
void premultiplycolor(pgfxvertex v, std::int32_t n)
{
    std::int32_t i;
    float a;
    ...
    else
    {
        ...
        for (j = 1; j <= h; j++)
        {
            ...
            for (i = 1; i <= wb; i++)
            {
                ...
                {
                    pixel =
                        0xff & ((std::uint32_t)0xff00 >> ((((std::uint32_t)*src >> k) & 1) << 3));
                    *dst = pixel;
                    dst += 1;
                    *dst = pixel;
                    dst += 1;
...
void tgfxspritesheet::loadnextimage()
{
    ...
    if (ld->images[i] == nullptr)
    {
        ...
        {
            ld->imagestargetscale[i] =
                max((float)(ld->imagestargetsize[i].x) / ld->images[i]->width(),
