...
constexpr std::array<std::int32_t, 4> OPENGL_TEXTURE_FORMAT = {
	{ GL_ALPHA, GL_RG, GL_RGB, GL_RGBA }
};

static bool gOpenGLES = false;
static auto IsOpenGLES() -> bool
{
	return gOpenGLES;
}
...
static auto initshaderprogram(bool dithering) -> bool
{
	...
	requiredfunctions = assigned(glCreateShader) && assigned(glShaderSource) && assigned(glCompileShader)
...
auto gfxinitcontext(SDL_Window* wnd, bool dithering, bool fixedpipeline) -> bool
{
	...
	requiredfunctions = assigned(glBindBuffer) && // vertex/index buffers
					 ...
					 && assigned(glTexImage2D) && assigned(glTexSubImage2D) && assigned(glEnable) && // general
						assigned(glDisable) && assigned(glBlendFunc) && assigned(glPixelStorei) && assigned(glViewport)
					 && assigned(glClearColor) && assigned(glClear) && assigned(glDrawArrays)
	...
	if (fixedpipeline)
	{
		requiredfunctions = assigned(glEnableClientState) && assigned(glVertexPointer) && assigned(glTexCoordPointer)
						 && assigned(glColorPointer) && assigned(glLoadMatrixf);
...
void gfxblit(tgfxtexture* src, tgfxtexture* dst, trect srcrect, trect dstrect, tgfxtexturefilter filter)
{
	GLenum filtergl;
	...
	glBlitFramebuffer(srcrect.left,
		srcrect.bottom,
		srcrect.right,
		srcrect.top,
		dstrect.left,
...
void gfxdraw(tgfxvertexbuffer* buffer, tgfxindexbuffer* indexbuffer, pgfxdrawcommand cmds, std::int32_t cmdcount)
{
	std::int32_t i;
	...
	{
		gfxbindtexture(cmds->texture);
		glDrawElements(GL_TRIANGLES, cmds->count, GL_UNSIGNED_SHORT, (void*)(sizeof(std::uint16_t) * cmds->offset));
		cmds += 1;
	}
}
...
std::unique_ptr<std::uint8_t[]> gfxsavescreen(std::int32_t x, std::int32_t y, std::int32_t w, std::int32_t h)
{
	...
	{
		std::swap_ranges(
			data.get() + 4 * w * line, data.get() + 4 * w * (line + 1), data.get() + 4 * w * (h - line - 1));
	}
	return data;
}

void gfxsavescreen(
	const std::string& filename, std::int32_t x, std::int32_t y, std::int32_t w, std::int32_t h, bool async)
{
	std::uint8_t* data;
	std::uint8_t* src;
...
void gfxgeneratemipmap(tgfxtexture* texture)
{
	...
	else
	{
		...
		glTexSubImage2D(GL_TEXTURE_2D,
			...
			1,
			1,
			OPENGL_TEXTURE_FORMAT[texture->fcomponents - 1],
			GL_UNSIGNED_BYTE,
...
auto addglyphtable(pfont f, std::int32_t fontsize, std::int32_t stretch, std::uint32_t flags) -> std::int32_t
{
	std::int32_t i;
...
auto findglyphtable(pfont f, std::int32_t fontsize, std::int32_t stretch, std::uint32_t flags) -> std::int32_t
{
	std::int32_t result;
	...
	const std::string_view& debug_name)
{
	...
	if (msaa && (gfxcontext.msaasamples > 0))
	{
		...
		glTexImage2DMultisample(
			GL_TEXTURE_2D_MULTISAMPLE, gfxcontext.msaasamples, OPENGL_TEXTURE_FORMAT[comp], width, height, 1u);

		glGenFramebuffers(1, &ffbohandle);
		glBindFramebuffer(GL_FRAMEBUFFER, ffbohandle);
		glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D_MULTISAMPLE, fhandle, 0);
		glBindFramebuffer(GL_FRAMEBUFFER, 0);
	}
	else
	{
...
void tgfxtexture::update(std::int32_t x, std::int32_t y, std::int32_t w, std::int32_t h, std::uint8_t* data)
{
	...
	glBindTexture(GL_TEXTURE_2D, fhandle);
	glTexSubImage2D(GL_TEXTURE_2D, 0, x, y, w, h, OPENGL_TEXTURE_FORMAT[fcomponents - 1], GL_UNSIGNED_BYTE, data);
...
void tgfxvertexbuffer::update(std::int32_t offset, std::int32_t count, pgfxvertex data) const
{
	...
	glBindBuffer(GL_ARRAY_BUFFER, fhandle);
	glBufferSubData(GL_ARRAY_BUFFER, static_cast<GLintptr>(size * offset), static_cast<GLsizeiptr>(size * count), data);
}
...
void tgfxindexbuffer::update(std::int32_t offset, std::int32_t count, uint16_t* data) const
{
	...
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, fhandle);
	glBufferSubData(
		GL_ELEMENT_ARRAY_BUFFER, static_cast<GLintptr>(size * offset), static_cast<GLsizeiptr>(size * count), data);
}
