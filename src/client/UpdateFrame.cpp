...
void update_frame(std::shared_ptr<FLogFile>& InConsoleLogFile)
{
	...
	if (game.GetMapchangecounter() < 0)
	{
		...
		if (game.GetMainTickCounter() % (second * 5) == 0)
		{
			if ((gGlobalStateClient.camerafollowsprite > 0)
				...
				&& (CVar::sv_survivalmode) && !game.GetSurvivalEndRound())
			{
	...
	if ((sprite_system.IsPlayerSpriteValid()) && (!gGlobalStateDemo.demoplayer.active()))
	{
		...
		{
			j = sprite.num;
			if (sprite.isnotspectator() && (!sprite_system.IsPlayerSprite(j)) && (sprite.bonusstyle != bonus_predator)
				and ((sprite.position == pos_stand)
					 or (sprite.isnotsolo() && sprite.isinsameteam(sprite_system.GetPlayerSprite()))
					 or sprite_system.GetPlayerSprite().deadmeat or sprite.deadmeat)
				and ((sprite.visible > 40) or (!CVar::sv_realisticmode)))
			{
				const auto& spritePartsPos = sprite_system.GetSpritePartsPos(j);
				if (distance(-gGlobalStateGame.gamewidthhalf + gGlobalStateClient.camerax + gGlobalStateClientGame.mx,
						-gGlobalStateGame.gameheighthalf + gGlobalStateClient.cameray + gGlobalStateClientGame.my,
						spritePartsPos.x,
						spritePartsPos.y)
					< cursorsprite_distance)
				{
					...
					if (game.isteamgame())
					{
						if (sprite.isinsameteam(sprite_system.GetPlayerSprite()))
						{
							gGlobalStateInterfaceGraphics.cursortext =
								gGlobalStateInterfaceGraphics.cursortext + ' '
								+ inttostr(std::round((sprite.GetHealth() / game.GetStarthealth()) * 100)) + '%';
	...
	else if (game.GetBulletTimeTimer() < 1)
	{
		...
		for (j = 0; j < max_big_messages; j++)
		{
			...
			{
				gGlobalStateInterfaceGraphics.worlddelay[j] = gGlobalStateInterfaceGraphics.worlddelay[j] - 1;
			}
		}
	...
	// MOVE -=CAMERA=-
	if ((gGlobalStateClient.camerafollowsprite > 0) && (gGlobalStateClient.camerafollowsprite < max_sprites + 1))
	{
	...
	else if (gGlobalStateClient.camerafollowsprite == 0)
	{
		...
		if ((gGlobalStateClientGame.mx > 310 * displayratio) && (gGlobalStateClientGame.mx < 330 * displayratio)
			&& (gGlobalStateClientGame.my > 230) && (gGlobalStateClientGame.my < 250))
		{
			m.x = 0;
			m.y = 0;
		}
