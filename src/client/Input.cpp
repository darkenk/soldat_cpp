...
auto GlobalStateInput::bindkey(
	const std::string& key, const std::string& action, const std::string& command, std::uint32_t modifier) -> bool
{
	...
	LogInfo(
		LOG, "BindKey id: {} Key: {} ({}), Mod: {} Command: {}", binds.size(), key, bind.keyid, bind.keymod, command);
...
void GlobalStateInput::unbindall()
{
	binds.clear();
}
