...
constexpr std::array<SDL_GPUTextureFormat, 4> SDL_GPU_TEXTURE_FORMAT = {
	{ SDL_GPU_TEXTUREFORMAT_A8_UNORM,
		SDL_GPU_TEXTUREFORMAT_R8G8_UNORM, SDL_GPU_TEXTUREFORMAT_INVALID,
...
static auto join1(const std::array<std::string, N>& list, const std::string& separator) -> string
{
	...
	std::copy(list.begin(), list.end(), std::ostream_iterator<const std::string&>(os, separator.c_str()));
	return os.str();
}
...
auto rectwidth(const tgfxrect& rect) -> float
{
	return std::abs(rect.right - rect.left);
}

auto rectheight(const tgfxrect& rect) -> float
{
...
static constexpr std::uint32_t gfxCalculateSamplerIndex(const SDL_GPUFilter min_filter, const SDL_GPUFilter mag_filter)
{
...
static SDL_GPUSampler* gfxCreateSampler(const SDL_GPUFilter min_filter, const SDL_GPUFilter mag_filter)
{
	...
	SDL_SetStringProperty(sampler_info.props, SDL_PROP_GPU_SAMPLER_CREATE_NAME_STRING, sampler_name.c_str());
...
static SDL_GPUGraphicsPipeline* gfxCreateGraphicsPipeline()
{
	...
	color_target_desc[0].format = SDL_GetGPUSwapchainTextureFormat(gfxcontext.mGpuDevice,
		gfxcontext.mWindow); // TODO(vscode): This is a hack, we should have a way to get the swapchain format
	...
	SDL_SetStringProperty(
		pipeline_info.props, SDL_PROP_GPU_GRAPHICSPIPELINE_CREATE_NAME_STRING, "soldat_graphics_pipeline");
...
#pragma region sdl3
	...
	{
		gfxcontext.mGpuDevice = SDL_CreateGPUDevice(
		...
		if (wnd != nullptr)
		{
			AbortIf(!SDL_ClaimWindowForGPUDevice(gfxcontext.mGpuDevice, wnd),
...
#pragma endregion sdl3
	...
	SDL_SetStringProperty(vertex_shader_info.props, SDL_PROP_GPU_SHADER_CREATE_NAME_STRING, "soldat_vertex_shader");
	...
	SDL_SetStringProperty(fragment_shader_info.props, SDL_PROP_GPU_SHADER_CREATE_NAME_STRING, "soldat_fragment_shader");
	...
	gfxcontext.mSamplers[gfxCalculateSamplerIndex(SDL_GPU_FILTER_NEAREST, SDL_GPU_FILTER_NEAREST)] =
		gfxCreateSampler(SDL_GPU_FILTER_NEAREST, SDL_GPU_FILTER_NEAREST);
	gfxcontext.mSamplers[gfxCalculateSamplerIndex(SDL_GPU_FILTER_NEAREST, SDL_GPU_FILTER_LINEAR)] =
		gfxCreateSampler(SDL_GPU_FILTER_NEAREST, SDL_GPU_FILTER_LINEAR);
	gfxcontext.mSamplers[gfxCalculateSamplerIndex(SDL_GPU_FILTER_LINEAR, SDL_GPU_FILTER_NEAREST)] =
		gfxCreateSampler(SDL_GPU_FILTER_LINEAR, SDL_GPU_FILTER_NEAREST);
	gfxcontext.mSamplers[gfxCalculateSamplerIndex(SDL_GPU_FILTER_LINEAR, SDL_GPU_FILTER_LINEAR)] =
		gfxCreateSampler(SDL_GPU_FILTER_LINEAR, SDL_GPU_FILTER_LINEAR);

	// create a default white texture

	color.rgba = 0xffffffff;
	gfxcontext.whitetexture = gfxcreatetexture(1, 1, 4, reinterpret_cast<std::uint8_t*>(&color), "white texture");
...
void gfxtarget(tgfxtexture* rendertarget)
{
	...
	if (rendertarget == nullptr)
	{
		if (gfxcontext.mSwapchainTexture == nullptr)
		{
			...
			SDL_WaitAndAcquireGPUSwapchainTexture(gfxcontext.mCommandBuffer,
				gfxcontext.mWindow,
				&gfxcontext.mSwapchainTexture,
...
void gfxblit(
	tgfxtexture* /*src*/, tgfxtexture* /*dst*/, trect /*srcrect*/, trect /*dstrect*/, tgfxtexturefilter /*filter*/)
{
...
void gfxdraw(tgfxvertexbuffer* buffer, std::int32_t offset, std::int32_t count)
{
	...
	if (gfxcontext.mTransformDirty)
	{
		SDL_PushGPUVertexUniformData(
			gfxcontext.mCommandBuffer, 0, gfxcontext.mTransform.data(), sizeof(gfxcontext.mTransform));
...
void gfxdraw(tgfxvertexbuffer* buffer, tgfxindexbuffer* indexbuffer, std::int32_t offset, std::int32_t count)
{
	...
	gfxcontext.mClearColorDirty = false;
	{
		...
		SDL_BindGPUIndexBuffer(
			gfxcontext.mRenderPass, &buffer_binding, SDL_GPUIndexElementSize::SDL_GPU_INDEXELEMENTSIZE_16BIT);
...
void gfxdraw(
	tgfxvertexbuffer* /*buffer*/, tgfxindexbuffer* /*indexbuffer*/, pgfxdrawcommand /*cmds*/, std::int32_t /*cmdcount*/)
{
	NotImplemented("rendering");
}
...
void gfxsavescreen(const std::string& /*filename*/,
	std::int32_t /*x*/,
	std::int32_t /*y*/,
	std::int32_t /*w*/,
	std::int32_t /*h*/,
...
auto argb(std::uint32_t argb) -> tgfxcolor
{
...
void gfxbindtexture(tgfxtexture* texture)
{
	...
	gfxcontext.mTextureSamplerBinding.sampler =
		gfxcontext.mSamplers[gfxCalculateSamplerIndex(tex->mMinFilter, tex->mMagFilter)];
	gfxcontext.mTextureSamplerBinding.texture = tex->getTexture();
}

auto gfxcreatetexture(
	std::int32_t w, std::int32_t h, std::int32_t c, std::uint8_t* data, const std::string_view& debug_name)
	-> tgfxtexture*
{
	return new tgfxtexture(w, h, c, false, false, data, debug_name);
}

auto gfxcreaterendertarget(std::int32_t w, std::int32_t h, std::int32_t c, bool msaa) -> tgfxtexture*
{
...
void gfxupdatetexture(
	tgfxtexture* texture, std::int32_t x, std::int32_t y, std::int32_t w, std::int32_t h, std::uint8_t* data)
{
...
auto gfxcreateindexbuffer(std::int32_t capacity, bool _static, std::uint16_t* data) -> tgfxindexbuffer*
{
...
void gfxdrawquad(
	tgfxtexture* texture, const tgfxvertex& a, const tgfxvertex& b, const tgfxvertex& c, const tgfxvertex& d)
{
...
void gfxspritevertices(pgfxsprite s,
	float x,
	float y,
	float w,
	float h,
...
void gfxdrawsprite(pgfxsprite s, float x, float y, float sx, float sy, float rx, float ry, float r, const tgfxrect& rc)
{
...
void gfxdrawsprite(pgfxsprite s, float x, float y, float rx, float ry, float r, const tgfxcolor& color)
{
...
void gfxdrawsprite(pgfxsprite s, float x, float y, float scale, const tgfxcolor& color, const tgfxrect& rc)
{
	gfxdrawsprite(s, x, y, scale, scale, 0, 0, 0, color, rc);
}

void gfxdrawsprite(pgfxsprite s, float x, float y, float sx, float sy, const tgfxcolor& color, const tgfxrect& rc)
{
	gfxdrawsprite(s, x, y, sx, sy, 0, 0, 0, color, rc);
}

void gfxdrawsprite(
	pgfxsprite s, float x, float y, float rx, float ry, float r, const tgfxcolor& color, const tgfxrect& rc)
{
	gfxdrawsprite(s, x, y, 1, 1, rx, ry, r, color, rc);
}

void gfxdrawsprite(
	pgfxsprite s, float x, float y, float sx, float sy, float rx, float ry, float r, const tgfxcolor& color)
{
	std::vector<tgfxvertex> v{ 4 };
...
void gfxdrawsprite(pgfxsprite s,
	float x,
	float y,
	float sx,
	float sy,
...
static auto addglyphtable(pfont f, std::int32_t fontsize, std::int32_t stretch, std::uint32_t flags) -> std::int32_t
{
	std::int32_t i = 0;
...
static auto findglyphtable(pfont f, std::int32_t fontsize, std::int32_t stretch, std::uint32_t flags) -> std::int32_t
{
	std::int32_t result = 0;
		...
		for (j = 1; j <= h; j++)
		{
			...
			for (i = 1; i <= wb; i++)
			{
				for (k = 7; k >= 0; k--)
				{
					pixel =
						0xff
						& (static_cast<std::uint32_t>(0xff00) >> (((static_cast<std::uint32_t>(*src) >> k) & 1) << 3));
					*dst = pixel;
					dst += 1;
					*dst = pixel;
					dst += 1;
			...
			{
				pixel =
					0xff & (static_cast<std::uint32_t>(0xff00) >> (((static_cast<std::uint32_t>(*src) >> k) & 1) << 3));
				*dst = pixel;
				dst += 1;
				*dst = pixel;
				dst += 1;
...
static auto iswhitespace(char ch) -> bool
{
	return (ch == '\12') || (ch == '\15');
}
...
{
	std::int32_t result = 0;
	result = findglyphtable(static_cast<pfont>(font), trunc(fontsize * 1000), trunc(stretch * 1000), flags);
...
void gfxsetfonttable(tgfxfont font, std::int32_t tableindex)
{
	...
	requestfontsize(static_cast<pfont>(font), gfxcontext.glyphtable->fontsize, gfxcontext.glyphtable->stretch);
}
...
auto gfxmat3transform(float tx, float ty, float sx, float sy, float cx, float cy, float r) -> tgfxmat3
{
...
tgfximage::tgfximage(const std::string& filename, tgfxcolor colorkey)
{
	...
	{
		fdata = stbi_load_from_memory(
			filebuffer.data(), length(filebuffer), &fwidth, &fheight, &fnumframes, STBI_rgb_alpha);
	}
	else
	{
		fdata = nullptr;
...
void tgfximage::update(
	std::int32_t x, std::int32_t y, std::int32_t w, std::int32_t h, std::uint8_t* data, std::int32_t frame)
{
	std::uint8_t* src = nullptr;
	std::uint8_t* dst = nullptr;
	std::int32_t i = 0;
...
tgfxtexture::tgfxtexture(std::int32_t width,
	std::int32_t height,
	std::int32_t comp,
	bool rt,
	bool msaa,
	...
	: fhandle(1), ffbohandle(0), fwidth(width), fheight(height), fcomponents(comp), fsamples(0)
{
	...
	// Create the Image:
	{
		...
		SDL_SetStringProperty(texture_info.props, SDL_PROP_GPU_TEXTURE_CREATE_NAME_STRING, debug_name.data());

		mTexture = SDL_CreateGPUTexture(gfxcontext.mGpuDevice, &texture_info);
		AbortIf(mTexture == nullptr, "Failed to create texture {}. Error {}", debug_name, SDL_GetError());
	...
	if (data != nullptr)
	{
		...
		AbortIf(transferbuffer == nullptr,
			"Failed to create transfer buffer to upload {}. Error {}",
			debug_name,
			SDL_GetError());
...
void tgfxtexture::update(std::int32_t x, std::int32_t y, std::int32_t w, std::int32_t h, std::uint8_t* data)
{
	SDL_GPUTransferBufferCreateInfo transferbuffer_info = {};
	transferbuffer_info.usage = SDL_GPU_TRANSFERBUFFERUSAGE_UPLOAD;
...
tgfxvertexbuffer::tgfxvertexbuffer(std::int32_t cap, bool /*_static*/, pgfxvertex data) : fcapacity(cap)
{
	...
	SDL_GPUTransferBuffer* vertex_transferbuffer =
		SDL_CreateGPUTransferBuffer(gfxcontext.mGpuDevice, &vertex_transferbuffer_info);
	...
	auto* vtx_dst =
		static_cast<pgfxvertex>(SDL_MapGPUTransferBuffer(gfxcontext.mGpuDevice, vertex_transferbuffer, true));
...
tgfxindexbuffer::tgfxindexbuffer(std::int32_t cap, bool /*_static*/, uint16_t* data) : fcapacity(cap)
{
	SDL_GPUBufferCreateInfo buffer_info = {};
	...
	auto* vtx_dst =
		static_cast<uint16_t*>(SDL_MapGPUTransferBuffer(gfxcontext.mGpuDevice, vertex_transferbuffer, true));
	memcpy(vtx_dst, data, buffer_info.size);
	SDL_UnmapGPUTransferBuffer(gfxcontext.mGpuDevice, vertex_transferbuffer);
...
void tgfxindexbuffer::update(std::int32_t offset, std::int32_t count, uint16_t* data) const
{
	...
	auto* vtx_dst = static_cast<uint16_t*>(SDL_MapGPUTransferBuffer(gfxcontext.mGpuDevice, transferbuffer, true));
...
auto tgfxspritesheet::getsprite(std::int32_t index) -> pgfxsprite
{
	return &fsprites[index];
}

auto tgfxspritesheet::gettexture(std::int32_t index) -> tgfxtexture*
{
	return ftextures[index];
}

auto tgfxspritesheet::getspritecount() -> std::int32_t
{
	return length(fsprites);
}

auto tgfxspritesheet::gettexturecount() -> std::int32_t
{
...
void tgfxspritesheet::updatenextsprite()
{
	...
	ld->atlasimages[info->texture]->update(x, y, w, h, ld->images[info->image]->getimagedata(info->frame));
	...
	sprite->texcoords.right = static_cast<float>((sprite->x + sprite->width)) / sprite->texture->width();
	sprite->texcoords.bottom = static_cast<float>((sprite->y + sprite->height)) / sprite->texture->height();
...
void tgfxspritesheet::updatetexture()
{
	...
	{
		gfxupdatetexture(ftextures[i],
			0,
			0,
			ld->atlasimages[i]->width(),
			ld->atlasimages[i]->height(),
