...
template <typename... Args>
inline void LogDebug(const std::string_view& logger, fmt::format_string<Args...> fmt, Args&&... args)
{
...
template <typename... Args>
inline void LogError(const std::string_view& logger, fmt::format_string<Args...> fmt, Args&&... args)
{
	spdlog::get(logger.data())->error(fmt, std::forward<Args>(args)...);
}

template <typename... Args>
inline void LogCritical(const std::string_view& logger, fmt::format_string<Args...> fmt, Args&&... args)
{
