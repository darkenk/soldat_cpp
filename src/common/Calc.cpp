...
auto islineintersectingcircle(tvector2 line1, tvector2 line2, tvector2 circlecenter, float radius)
	-> tintersectionresult
{
	MyFloat a = 0.F;
	MyFloat a1 = 0.F;
...
auto linecirclecollision(const tvector2& startpoint,
	const tvector2& endpoint,
	const tvector2& circlecenter,
	float radius,
...
auto pointlinedistance(const tvector2& p1, const tvector2& p2, const tvector2& p3) -> float
{
	...
	u = ((p3.x - p1.x) * (p2.x - p1.x) + (p3.y - p1.y) * (p2.y - p1.y)) / (sqr(p2.x - p1.x) + sqr(p2.y - p1.y));
...
auto roundfair(float value) -> std::int32_t
{
