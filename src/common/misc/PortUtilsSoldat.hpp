...
public:
	...
	operator const char*() { return ref.c_str(); }
...
public:
	...
	{
		constexpr SourceLocation(const T& v, const source_location& location = source_location::current())
	...
	constexpr std::int32_t StartIdx() const { return StartIndex; }

	constexpr std::int32_t EndIdx() const { return EndIndex; }
...
{
public:
	void add(const std::string& v) { push_back(v); }
	void savetofile(const std::string& filename) { NotImplemented("filesystem", "lack of save for tstringlist"); }
	void loadfromfile(const std::string& filename) { NotImplemented("filesystem", "lack of load for tstringlist"); }
};
...
{
	return fmt::vformat(std::locale("en_US.UTF-8"), std::string_view(fmt), fmt::make_format_args(args...));
}
