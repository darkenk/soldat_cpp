...
namespace
{
	auto SerializePolygonType(const std::uint8_t polygonType) -> PolygonType
	{
...
void Polymap::loaddata(const tmapfile& mapfile)
{
	...
	if (this->polycount > 0)
	{
		std::memcpy(&this->polys[1], mapfile.polygons.data(), sizeof(tmappolygon) * this->polycount);
	}

	if (this->collidercount > 0)
	{
		std::memcpy(&this->collider[1], mapfile.colliders.data(), sizeof(tmapcollider) * this->collidercount);
	}

	if (length(mapfile.spawnpoints) > 0)
	{
		std::memcpy(
			&this->spawnpoints[1], mapfile.spawnpoints.data(), sizeof(tmapspawnpoint) * length(mapfile.spawnpoints));
	}

	if (length(mapfile.waypoints) > 0)
	{
		NotImplemented("map");
		std::memcpy(&botpath.waypoint[1], mapfile.waypoints.data(), sizeof(twaypoint) * length(mapfile.waypoints));
	}
	...
	{
		if ((std::abs(this->spawnpoints[i].x) >= 2000000) || (std::abs(this->spawnpoints[i].y) >= 2000000))
		{
	...
	{
		if ((std::abs(botpath.waypoint[i].x) >= 2000000) || (std::abs(botpath.waypoint[i].y) >= 2000000))
		{
...
auto Polymap::loadmap(const tmapfile& mapfile) -> bool
{
	initialize();
...
static auto LineInPoly(
	const tvector2& a, const tvector2& b, const PolyMapSector::Poly& poly, tvector2& v, const float ak) -> bool
{
	...
	auto calc = [&](const PolyMapSector::Vertex& p, const PolyMapSector::Vertex& q)
	{
		if (b.x != a.x && q.x != p.x)
		{
			...
			if (ak != bk)
			{
				...
				if ((v.x > std::min(p.x, q.x)) && (v.x < std::max(p.x, q.x)) && (v.x > std::min(a.x, b.x))
					&& (v.x < std::max(a.x, b.x)))
				{
					return true;
				}
...
auto Polymap::pointinpoly(const tvector2& p, const tmappolygon& poly) -> bool
{
...
auto Polymap::PointInPoly(const tvector2& p, const PolyMapSector::Poly& poly) -> bool
{
	ZoneScopedN("PointInPoly");
...
auto Polymap::closestperpendicular(std::int32_t j, const tvector2& pos, float& d, std::int32_t& n) -> tvector2
{
...
auto Polymap::collisiontestexcept(const tvector2& pos, tvector2& perpvec, std::int32_t c) -> bool
{
...
auto Polymap::ShouldTestPolygonWithRay(
	const std::uint8_t polygonType, const bool npcol, const bool nbcol, const bool flag, const std::uint8_t team)
	-> bool
{
	switch (polygonType)
	{
...
auto Polymap::GetSectorCoordUnsafe(const tvector2& pos) const -> Polymap::SectorCoord
{
...
{
	if ((pos.x >= MapHalfSize || pos.x <= -MapHalfSize) || (pos.y >= MapHalfSize || pos.y <= -MapHalfSize))
	{
...
auto Polymap::RayCastOpt(const tvector2& a,
	const tvector2& b,
	float& distance,
	float maxdist,
	bool player,
...
auto Polymap::raycast(const tvector2& a,
	const tvector2& b,
	float& distance,
	float maxdist,
	bool player,
...
void Polymap::checkoutofbounds(MyFloat& x, MyFloat& y) const
{
...
void Polymap::checkoutofbounds(std::int16_t& x, std::int16_t& y) const
{
	if (x < (10 * (-sectorsnum * SectorsDivision) + 50))
	{
		x = 1;
