...
using tloadmapgraphics =
	std::function<void(tmapfile& mapfile, bool bgforce, tmapcolor bgcolortop, tmapcolor bgcolorbtm)>;
...
public:
	struct Sector
	{
		inline bool IsValid() const { return Polys != nullptr; }

		inline const PolyMapSector::TPolys& GetPolys() const { return *Polys; }

		static inline Sector CreateInvalid() { return { nullptr }; }
	...
	static bool ShouldTestPolygonWithRay(
		const uint8_t polygonType, const bool npcol, const bool nbcol, const bool flag, const uint8_t team);
...
private:
	...
	twaypoints& botpath;
	bool RayCastOpt(const tvector2& a,
		const tvector2& b,
		float& distance,
		float maxdist,
		bool player = false,
