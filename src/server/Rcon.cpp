...
void tadminserver::handleconnect(tidcontext athread)
{
	...
	else
	{
		...
		scrptdispatcher.onadminconnect(
			tidiohandlersocket(iohandler).binding.peerip, tidiohandlersocket(iohandler).binding.peerport);
#endif
	}
	//  except
	//    on e: Exception do
...
void tadminserver::handledisconnect(tidcontext athread)
{
	...
	//  end;
#ifdef SCRIPT
	scrptdispatcher.onadmindisconnect(tidiohandlersocket(athread.connection.iohandler).binding.peerip,
