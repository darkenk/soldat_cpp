  ...
  else if (msg->msg == "REFRESHX")
  {
    ...
    for (i = 1; i <= max_players; i++)
      if (sortedplayers[i].playernum > 0)
      {
        ...
        refreshmsgx.caps[i] =
          SpriteSystem::Get().GetSprite(sortedplayers[i].playernum).player.flags;
        refreshmsgx.deaths[i] =
          SpriteSystem::Get().GetSprite(sortedplayers[i].playernum).player.deaths;
        refreshmsgx.ping[i] =
          SpriteSystem::Get().GetSprite(sortedplayers[i].playernum).player.pingtime;
        ...
        refreshmsgx.number[i] = SpriteSystem::Get().GetSprite(sortedplayers[i].playernum).num;
                ...
                if (SpriteSystem::Get()
                      .GetSprite(sortedplayers[i].playernum)
