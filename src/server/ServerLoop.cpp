...
void apponidle()
{
	...
	{ // frame rate independant code
		...
		if (GS::GetGame().GetMainTickCounter() % second == 0)
		{
								...
								|| ((sprite.player->realping < CVar::sv_minping) && (sprite.player->pingtime > 0))))
						{
							...
							if (gGlobalStateNetworkServer.pingwarnings[sprite.num] > CVar::sv_warnings_ping)
							{
								gGlobalStateServer.kickplayer(
									sprite.num, true, kick_ping, sixty_minutes / 4, "Ping Kick");
							}
						}
					}
				}
			...
			for (auto& sprite : sprite_system.GetActiveSprites())
			{
						...
						&& (gGlobalStateNetworkServer.messagesasecnum[j] > CVar::net_floodingpacketsinternet)))
				{
					...
					{
						gGlobalStateServer.kickplayer(j, true, kick_flooding, sixty_minutes / 4, "Flood Kicked");
					}
				}
			}
		...
		else if (CVar::net_lan == INTERNET)
		{
			...
			if (GS::GetGame().GetMainTickCounter()
					% static_cast<std::int32_t>(std::round(CVar::net_t1_heartbeat * adjust))
				== 0)
			{
				serverheartbeat(*gGlobalStateNetworkServer.GetServerNetwork(), sprite_system, GS::GetGame());
			}
		...
		for (auto& sprite : sprite_system.GetActiveSprites())
		{
			if ((sprite.player->controlmethod == human) && (sprite.player->port > 0))
			{
				...
				if (CVar::ac_enable)
				{
					...
					else if ((GS::GetGame().GetMainTickCounter() % (second * 3) == 0)
							 and (!SpriteSystem::Get().GetSprite(j).player->faeresponsepending))
					{
				...
				{
					if (GS::GetGame().GetMainTickCounter()
							% static_cast<std::int32_t>(std::round(CVar::net_t1_ping * adjust))
						== 0)
					{
						serverping(j);
					}
				...
				for (auto& sprite : sprite_system.GetActiveSprites())
				{
						...
						&& (sprite.player->grabbedinbase))
					{
						...
						if (!scrptdispatcher.onvotekickstart(255, j, "Server: Possible cheating"))
						{
							...
							serversendvoteon(
								GS::GetGame().GetVoteType(), 255, inttostr(j), "Server: Possible cheating");
#ifdef SCRIPT
						}
#endif
						GS::GetMainConsole().console(
		...
		if (GS::GetGame().GetMainTickCounter() % second == 0)
		{
			for (auto& sprite : sprite_system.GetActiveSprites())
			{
				if (sprite.player->chatwarnings > 5)
				{
					...
					gGlobalStateServer.kickplayer(sprite.num, true, kick_flooding, five_minutes, "Chat Flood");
				}
						...
						if ((sprite.holdedthing > 0))
						{
							if (things[sprite.holdedthing].style == object_pointmatch_flag)
							{
								GS::GetGame().SetTeamScore(
									sprite.player->team, GS::GetGame().GetTeamScore(sprite.player->team) + 1);
