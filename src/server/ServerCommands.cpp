...
namespace
{
	...
	void commandban(std::vector<std::string>& args, std::uint8_t sender)
	{
		...
		for (i = 0; i <= high(targets); i++)
		{
			gGlobalStateServer.kickplayer(
				targets[i], true, kick_console, (day * 30), std::string("Banned by ") + tempstr);
		}
	}
	...
	void commandunbanlast(std::uint8_t sender)
	{
	...
	void commandnetstats(std::vector<std::string>& /*args*/, std::uint8_t /*sender*/)
	{
		...
		{
			auto statstext = gGlobalStateNetworkServer.GetServerNetwork()->GetDetailedConnectionStatus(dstplayer->peer);
			{
				LogInfo("network",
					"----------------\n{}\n{}\n{}\n----------------",
					dstplayer->name,
	...
	void commandinfo(std::vector<std::string>& /*args*/, std::uint8_t sender)
	{
		...
		serversendstringmessage(
			string("Address: ") + (gGlobalStateServer.serverip) + ':' + (inttostr(gGlobalStateServer.serverport)),
			sender,
			255,
			msgtype_pub);
		...
		serversendstringmessage(
			string("Timelimit: ") + (inttostr(CVar::sv_timelimit / 3600)), sender, 255, msgtype_pub);
		serversendstringmessage(string("Nextmap: ") + (checknextmap()), sender, 255, msgtype_pub);

#ifdef SCRIPT
		serversendstringmessage(
			string("Scripting: ") + iif(CVar::sc_enable, "Enabled", "Disabled"), sender, 255, msgtype_pub);
		if (CVar::sc_enable)
		{
			gametype = "";
		...
		{
			serversendstringmessage(string("Server uses weapon mod \"") + (gGlobalStateServer.wmname) + " v"
										+ (gGlobalStateServer.wmversion) + "\" (checksum "
	...
	void commandvotemap(std::vector<std::string>& args, std::uint8_t sender)
	{
		...
		{
			if (std::ranges::find(gGlobalStateServer.mapslist, mapname)
				!= gGlobalStateServer.mapslist.end()) /*and (MapExists(MapName, userdirectory))*/
			{
				...
				{
					serversendstringmessage(
						"Can't vote for 2:00 minutes after joining game or last vote", sender, 255, msgtype_pub);
				}
			}
			else
			{
				serversendstringmessage(string("Map not found (") + (mapname) + ')', sender, 255, msgtype_pub);
			}
		}
	}
